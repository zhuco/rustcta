...
pub(crate) fn ApiKeysPanel(
    ...
    let status_source = data().credentials_status;
    let status_rows = CredentialStatusRowData::from_response(&status_source, lang);
    let panel_data = ExchangeCredentialPanelData::from_status(&status_source, lang);
    let selected_slot =
        selected_credential_slot(&selected_exchange, &selected_account, &selected_namespace);
                ...
                if !token_value.is_empty() {
                    match fetch_credential_status(&token_value).await {
                        Ok(status) => {
                            ...
                            next.credentials_status = status.clone();
    ...
    rsx! {
        section { id: "api-keys", class: "panel api-keys credential-status-panel",
            ...
            div { class: "api-key-section credential-boundary",
                    ...
                    div { class: "table-wrap",
                        table {
                            thead { tr {
                                ...
                                th { {s(lang, "account")} }
                                th { {s(lang, "credential_namespace")} }
                                th { {s(lang, "fields")} }
...
fn selected_credential_slot(exchange: &str, account: &str, namespace: &str) -> String {
