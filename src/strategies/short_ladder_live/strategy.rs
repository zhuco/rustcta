...
#[derive(Clone)]
pub struct ShortLadderLiveStrategy {
    pub(super) config: ShortLadderLiveConfig,
...
impl ShortLadderLiveStrategy {
    pub fn new(config: ShortLadderLiveConfig, deps: StrategyDeps) -> Result<Self> {
        ...
        for symbol_cfg in &config.symbols {
            if symbol_cfg.enabled {
                symbols.insert(
                    ...
                    SymbolState::new(symbol_cfg.symbol.clone()),
                );
            }
        }
        ...
        for (symbol, state) in &runtime.symbols {
            if let Some(short) = &state.short {
                positions.push(StrategyPosition {
                    symbol: symbol.clone(),
                    net_position: -short.quantity,
                    notional: short.current_notional,
                });
            }
        }
    ...
    pub(super) async fn mark_error(&self, message: String) {
        ...
        log::error!("ShortLadderLiveStrategy 错误: {}", message);
    }
}

impl Strategy for ShortLadderLiveStrategy {
    type Config = ShortLadderLiveConfig;
...
#[async_trait]
impl StrategyInstance for ShortLadderLiveStrategy {
    async fn start(&self) -> Result<()> {
        ...
        *self.started_at.write().await = Some(Instant::now());
        ...
        log::info!(
            "ShortLadderLiveStrategy 已启动，交易对数量={}",
            self.config
                .symbols
                .iter()
    ...
    async fn stop(&self) -> Result<()> {
        ...
        log::info!("ShortLadderLiveStrategy 已停止");
        Ok(())
    }
