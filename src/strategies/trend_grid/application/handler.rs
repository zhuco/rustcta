...
use crate::strategies::trend_grid::domain::config::{
    BatchSettings, GridManagement, SpacingType, TradingConfig, TrendAdjustment,
};
use crate::strategies::trend_grid::domain::state::{ConfigState, TrendStrength};
use crate::strategies::trend_grid::infrastructure::planner;

use rust_decimal::Decimal;

/// 成交处理器
pub(crate) struct TradeHandler {
    config_id: String,
            ...
            OrderSide::Buy => {
                ...
                let (sell_price, buy_price) = match spacing_type {
                    SpacingType::Arithmetic => {
                        // 等差网格
                        // 新卖单：成交价 + 间距（近端）
                        let sell_price = planner::round_price(
                            trade.price + spacing,
                        ...
                        let buy_price = if !buy_orders.is_empty() {
                            ...
                            let lowest_buy = buy_orders
                                ...
                                .unwrap();
                            planner::round_price(lowest_buy - spacing, state_guard.price_precision)
                        } else {
            ...
            OrderSide::Sell => {
                ...
                let (buy_price, sell_price) = match spacing_type {
                    SpacingType::Arithmetic => {
                        // 等差网格
                        // 新买单：成交价 - 间距（近端）
                        let buy_price = planner::round_price(
                            trade.price - spacing,
                            state_guard.price_precision,
                        );
