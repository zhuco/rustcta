...
struct ASState {
    ...
    active_sell_order: Option<String>,
    last_buy_price: f64,
    last_sell_price: f64,
    ...
    // 时间管理
    strategy_start_time: DateTime<Utc>,
    last_update_time: DateTime<Utc>,
    last_volatility_update: DateTime<Utc>,
}
...
impl AvellanedaStoikovStrategy {
    ...
    async fn check_risk_limits(&self) -> Result<bool> {
        ...
        // 检查未实现亏损
        if state.unrealized_pnl < -self.config.risk.max_unrealized_loss {
            log::warn!("⚠️ 达到最大未实现亏损限制: {:.2}", state.unrealized_pnl);
            return Ok(false);
        }

        // 检查日亏损
        if state.daily_pnl < -self.config.risk.max_daily_loss {
            log::warn!("⚠️ 达到最大日亏损限制: {:.2}", state.daily_pnl);
            return Ok(false);
        }

        // 检查库存限制
        let inventory_value = state.inventory.abs() * state.mid_price;
        if inventory_value > self.config.risk.inventory_risk.max_position_value {
            log::warn!("⚠️ 库存价值超限: {:.2}", inventory_value);
            return Ok(false);
        }

        // 检查波动率限制
        if state.volatility > self.config.risk.volatility_limits.max_volatility / 100.0 {
            log::warn!("⚠️ 波动率过高: {:.2}%", state.volatility * 100.0);
            return Ok(false);
        }

        // 波动率过低只记录日志，不阻止交易
        if state.volatility < self.config.risk.volatility_limits.min_volatility / 100.0
        ...
        // 检查连续亏损
        if state.consecutive_losses >= self.config.risk.emergency_stop.consecutive_losses {
            log::warn!("⚠️ 连续亏损次数过多: {}", state.consecutive_losses);
            return Ok(false);
        }
