...
mod hedged_grid {
    ...
    #[test]
    fn strict_pairing_repeated_open_fills_should_stop_at_total_notional_limit() {
        let mut config = test_config();
        config.grid.strict_pairing = true;
        config.grid.order_qty = Some(0.1);
        config.grid.levels_per_side = 3;
        config.risk.max_total_notional = 70.0;
        config.risk.max_net_notional = 1_000.0;
        config.grid.refill_open_slots_enabled = true;
        config.grid.normalize_open_grid_enabled = true;
        ...
        let open_notional = engine
            ...
            .sum::<f64>();
        let total_notional =
            (position.long_qty + position.short_qty) * position.mark_price + open_notional;
        assert!(
            total_notional <= 70.0 + 1e-6,
            "strict pairing open budget leaked: position={position:?} open_notional={open_notional}"
        );
    }
