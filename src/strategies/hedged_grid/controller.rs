...
pub(super) async fn run_loop(
    ...
    if let Some(external_ws_rx) = external_ws_rx {
        ...
        log::info!(
            "[hedged_grid] 使用共享WebSocket用户流 symbol={}",
        ...
        log::info!("[hedged_grid] WebSocket推送已禁用，使用轮询模式");
    }
            ...
            _ = poll_interval.tick() => {
                ...
                .await
                {
                    ...
                    Err(err) => {
                        ...
                        log::warn!("[hedged_grid] 行情拉取失败: {}", err);
                        continue;
                    }
                };
                    ...
                    >= ChronoDuration::milliseconds(config.polling.reconcile_interval_ms as i64)
                {
                    ...
                    let trades = if should_fetch_trades {
                        ...
                        .await
                        {
                            Ok(trades) => trades,
                            Err(err) => {
                                log::warn!("[hedged_grid] 成交记录拉取失败: {}", err);
                                Vec::new()
                            }
                        }
                    } else {
                    ...
                    if needs_hourly_reset {
                        ...
                        log::info!("[hedged_grid] 定时重置网格 interval=12h");
                    }
                    ...
                    if let Some((threshold, long_low, short_low, long_qty, short_qty)) = low_inventory_info {
                            ...
                            && now - last_zero_position_refresh >= zero_position_refresh_interval
                        {
                            ...
                            log::info!(
                                "[hedged_grid] 低仓刷新订单 threshold_qty={:.6} long_qty={:.6} short_qty={:.6} long_low={} short_low={}",
                                threshold,
                                long_qty,
                                short_qty,
                                long_low,
...
async fn drain_user_stream_events(
    ...
    *last_progress = Utc::now();
    log::debug!(
        "[hedged_grid] tick前优先处理WebSocket事件 count={}",
        drained
    );
    Ok(())
}
...
async fn handle_user_stream_event_batch(
    ...
    if fill_count > 1 || action_count > 0 {
        log::info!(
            "[hedged_grid] ws_micro_batch symbol={} events={} fills={} actions={} places={} cancels={} window_ms={} executor_ms={}",
            config.engine.symbol,
            processed.len(),
            fill_count,
            action_count,
    ...
    for item in processed {
        if let Some(fill) = &item.fill_info {
            log::info!(
                "[hedged_grid] latency_breakdown source=ws symbol={} order_id={} client_id={} ws_queue_ms={} engine_ms={} executor_ms={} total_ms={}",
                item.report_symbol,
    ...
    tokio::spawn(async move {
        ...
        while running.load(Ordering::SeqCst) {
            let Some(account) = account_manager.get_account(&account_id) else {
                log::warn!(
                    "[hedged_grid] WebSocket启动失败: 账户 {} 不存在",
                    account_id
                );
                sleep(backoff).await;
            ...
            if account.exchange_name.to_lowercase() != "binance" {
                log::info!(
                    "[hedged_grid] 当前交易所 {} 暂不支持用户流推送，跳过 WebSocket",
            ...
            let listen_key = match account.exchange.create_user_data_stream(market_type).await {
                Ok(key) => key,
                Err(err) => {
                    log::warn!("[hedged_grid] 获取 listenKey 失败: {}", err);
                    sleep(backoff).await;
                    backoff = (backoff * 2).min(max_backoff);
                    continue;
                }
            ...
            if let Err(err) = ws_client.connect().await {
                log::warn!("[hedged_grid] WebSocket连接失败: {}", err);
                sleep(backoff).await;
                backoff = (backoff * 2).min(max_backoff);
                continue;
            }
            ...
            log::info!(
                "[hedged_grid] WebSocket用户流已连接 (account={})",
                account_id
            );
                        ...
                        match msg {
                            Ok(Some(raw)) => match ws_client.parse_binance_message(&raw) {
                                ...
                                Ok(WsMessage::Error(err)) => {
                                    ...
                                    {
                                        log::warn!(
                                            "[hedged_grid] listenKey 已过期，准备重连"
                                        );
                                        break;
                                    }
                                }
                            ...
                            Err(err) => {
                                log::warn!("[hedged_grid] WebSocket接收失败: {}", err);
                                break;
                            }
                        }
                    }
...
fn log_position_snapshot(symbol: &str, position: &PositionState) {
    ...
    log::debug!(
        "[hedged_grid] 库存快照 {} long={:.6} short={:.6} long_entry={:.4} short_entry={:.4} long_avail={:.6} short_avail={:.6} net={:.6} mark={:.4} equity={:.2} mm={:.4}",
        symbol,
        position.long_qty,
        position.short_qty,
...
fn sync_orders(
    ...
    for client_id in engine.order_ids() {
        ...
        if let Some((qty, quote)) = trade_map.get(&exchange_id) {
            ...
            log::warn!(
                "[hedged_grid] 本地订单已连续缺失，按拒单处理 order_id={} exchange_id={} grace_ms={}",
                client_id,
                exchange_id,
            ...
            log::info!(
                "[hedged_grid] 本地订单从openOrders消失，等待WS成交或trade确认 order_id={} exchange_id={} grace_ms={}",
                client_id,
                exchange_id,
                MISSING_ORDER_GRACE_MS
            );
    ...
    while let Some(action) = cancel_queue.pop_front() {
        if let EngineAction::Cancel { order_id, reason } = action {
            if let Some(exchange_id) = order_map.exchange_id(&order_id) {
                ...
                log::warn!(
                    "[hedged_grid] 撤单跳过: 本地订单缺少exchange_id order_id={} reason={}",
                    order_id,
                    reason
                );
            }
        ...
        Ok(canceled_orders) => {
            ...
            for (order_id, exchange_id, reason) in cancel_items {
                if canceled_ids.contains(&exchange_id) {
                    ...
                    log::info!(
                        "[hedged_grid] 撤单成功 {} exchange_id={} reason={}",
                        order_id,
                        exchange_id,
                        reason
                    );
                    ...
                    log::warn!(
                        "[hedged_grid] 批量撤单未确认 {} exchange_id={} reason={}",
                        order_id,
                        exchange_id,
                        reason
                    );
            ...
            for (order_id, exchange_id, reason) in cancel_items {
                ...
                .await
                {
                    Ok(_) => {
                        order_map.remove_exchange(&exchange_id);
                        log::info!(
                            "[hedged_grid] 撤单成功 {} exchange_id={} reason={} fallback=true",
                            order_id,
                            exchange_id,
                            reason
                        );
                    ...
                    Err(err) => {
                        log::warn!(
                            "[hedged_grid] 撤单失败 {} exchange_id={} reason={} err={}",
                            order_id,
                            exchange_id,
                            reason,
                            err
        ...
        Ok(response) => {
            ...
            for failed in response.failed_orders {
                let Some(client_id) = failed.order_request.client_order_id.clone() else {
                    log::warn!(
                        "[hedged_grid] 批量下单失败条目缺少client_order_id: code={:?} msg={}",
                        failed.error_code,
                ...
                let Some(draft) = pending_by_id.remove(&client_id) else {
                    log::warn!(
                        "[hedged_grid] 批量失败条目无法匹配本地订单 id={} code={:?} msg={}",
                        client_id,
                        failed.error_code,
                        failed.error_message
                    );
    ...
    if expected_pairing_reject || post_only_reject {
        *failure_count = 0;
        if expected_pairing_reject {
            log::info!(
                "[hedged_grid] 严格配对下预期拒单(忽略重启) id={} intent={:?} qty={:.6} price={:.6} err={}",
                draft.id,
                draft.intent,
                draft.qty,
                draft.price,
            ...
            log::info!(
                "[hedged_grid] post-only拒单已重试(不触发重启) id={} intent={:?} qty={:.6} price={:.6} err={}",
                draft.id,
                draft.intent,
                draft.qty,
                draft.price,
        ...
        *failure_count += 1;
        log::warn!(
            "[hedged_grid] 下单失败 id={} intent={:?} side={:?} pos={} qty={:.6} price={:.6} post_only={} err={}",
            draft.id,
            draft.intent,
            draft.intent.side(),
            draft.intent.position_side().as_str(),
        ...
        match action {
            ...
            EngineAction::Place(draft) => {
                ...
                if canceled_ids.contains(&draft.id) {
                    log::debug!(
                        "[hedged_grid] 跳过同批次已取消挂单 id={} intent={:?}",
                        draft.id,
                        draft.intent
                    );
                    continue;
...
mod tests {
    use super::*;
    use crate::strategies::hedged_grid::engine::OrderDraft;
    use crate::strategies::hedged_grid::ledger::OrderIntent;

    use crate::strategies::hedged_grid::config::{
...
async fn restart_strategy(
    ...
    log::error!(
        "[hedged_grid] 检测到拒单/失败({})，等待30秒后重启策略",
        reason
    );
    let _ = with_timeout(
