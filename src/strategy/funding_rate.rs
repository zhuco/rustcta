...
use crate::exchange::binance_model::{ExchangeInfo, Filter, FundingRate};
...
impl FundingRateStrategy {
    ...
    pub async fn run(&mut self) -> Result<(), AppError> {
        ...
        strategy_info!(
            &self.config.name,
            "配置: 下单金额={}USDT, 费率阈值={:.4}%, 结算前{}ms开仓, 结算后{}ms平仓",
    ...
    async fn check_and_find_opportunity(&self) -> Result<(), AppError> {
        ...
        if let Some(opportunity) = best_opportunity {
            ...
            strategy_info!(
                ...
                opportunity.funding_rate * 100.0
            );
            strategy_info!(
                &self.config.name,
                "  下次结算时间: {}ms",
                opportunity.next_funding_time
            );
            // 执行套利
    ...
    async fn find_best_opportunity(
    ...
    ) -> Result<Option<ArbitrageOpportunity>, AppError> {
        ...
        let exchange_info = self
            .exchange
            .get_exchange_info(MarketType::UsdFutures)
            .await?;
        ...
        for rate_info in funding_rates {
            ...
            // 查找交易对信息获取精度
            if let Some(symbol_info) = exchange_info
                .symbols
                .iter()
                .find(|s| s.symbol == rate_info.symbol)
            {
    ...
    async fn execute_arbitrage(&self, opportunity: ArbitrageOpportunity) -> Result<(), AppError> {
        ...
        let side = if opportunity.funding_rate > 0.0 {
            "SELL"
        } else {
            "BUY"
        };
        ...
        let open_order = self
            .exchange
            .place_order(&symbol, side, "MARKET", quantity, None)
            .await?;
        ...
        let close_order = self
            .exchange
            .place_order(&symbol, close_side, "MARKET", quantity, None)
            .await?;
