...
use rustcta::market::{
    exchange_symbol_for, BookLevel, CanonicalSymbol, ExchangeId, MarketDataAdapter,
    MarketFundingSnapshot, MarketStateCache, OrderBook5, RouteStatus, RuntimeMode,
};
use rustcta::strategies::cross_exchange_arbitrage::{
    scan_opportunities, BundleReadModel, CrossArbDashboardStatus, CrossExchangeArbitrageConfig,
    MarketSnapshot, Opportunity, OpportunityReadModel, ReconcileReadModel, RiskEventReadModel,
    RouteReadModel, SimulatedBundleState, SimulatedBundleStatus,
};
use serde::Serialize;
use tokio::sync::mpsc;
...
impl AppState {
    fn new(config_path: impl AsRef<Path>, symbol_count: usize, data_source: DataSource) -> Self {
        Self {
...
impl CrossArbReadModel {
    fn new(config_path: &Path, symbol_count: usize, data_source: DataSource) -> Self {
        let now = Utc::now();
    ...
    fn simulate_synthetic_tick(&mut self, now: DateTime<Utc>) {
        ...
        for (index, symbol) in self.monitored_symbols.iter().enumerate() {
            ...
            let opportunities = scan_opportunities(symbol, &snapshots, &self.config, now);
            let best = opportunities.first().cloned();
            ...
            let symbol_analytics = SymbolAnalytics::from_best(symbol.clone(), best.as_ref(), now);
            symbols.push(symbol_analytics);
        ...
        let new_fills = if self.control.new_entries_allowed {
            all_opportunities
                .iter()
                .filter(|opportunity| opportunity.can_open)
                .take(DEFAULT_FILLS_PER_TICK)
                .map(|opportunity| self.fill_market_bundle(opportunity, now))
                .collect::<Vec<_>>()
        } else {
            Vec::new()
        };

        self.simulated_fills.extend(new_fills);
    ...
    fn apply_snapshots(
        ...
        for symbol in &self.monitored_symbols {
            ...
            symbols.push(SymbolAnalytics::from_best(
                symbol.clone(),
                best.as_ref(),
                now,
            ));
    ...
    fn fill_market_bundle(
        &self,
        opportunity: &Opportunity,
        now: DateTime<Utc>,
    ) -> SimulatedMarketFill {
        let gross_spread_pnl_usdt = opportunity.raw_open_spread * opportunity.target_notional_usdt;
        let fee_pnl_usdt = -(opportunity.open_fee_est_usdt + opportunity.close_fee_est_usdt);
        let funding_pnl_usdt = opportunity.expected_funding_usdt;
        let slippage_cost_usdt = opportunity.slippage_pct.abs() * opportunity.target_notional_usdt;
        let net_pnl_usdt =
            gross_spread_pnl_usdt + fee_pnl_usdt + funding_pnl_usdt - slippage_cost_usdt;
        let long_fill_price = if opportunity.raw_open_spread > -0.99 {
            Some(opportunity.maker_price / (1.0 + opportunity.raw_open_spread).max(0.0001))
        } else {
            None
        };
        let short_fill_price = Some(opportunity.maker_price);

        SimulatedMarketFill {
            trade_id: format!("sim-{}-{}", self.tick_index, opportunity.opportunity_id),
            bundle_id: format!("bundle-{}-{}", self.tick_index, opportunity.opportunity_id),
            ...
            fill_model: "market_all_filled".to_string(),
            opened_at: now,
            closed_at: now + Duration::milliseconds(250),
        }
    }
...
impl AnalyticsSummary {
    ...
    fn from_state(
        ...
        let total_notional_usdt = fills.iter().map(|fill| fill.target_notional_usdt).sum();
...
async fn dashboard(State(state): State<AppState>) -> Json<DashboardPayload> {
    ...
    Json(DashboardPayload {
        ...
        current_positions: current_positions_from_opportunities(&read_model.opportunities),
...
async fn main() -> Result<()> {
    ...
    let state = AppState::new(&args.config, args.symbol_count, args.data_source);
    if args.simulate {
        spawn_shadow_loop(
            state.clone(),
            args.refresh_secs,
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            match data_source {
                ...
                DataSource::PublicWs => {
                    let (config, symbols, cache, should_refresh_funding) = {
                        let read_model = state.read_model.read().await;
                        (
                            read_model.config.clone(),
                            read_model.monitored_symbols.clone(),
...
async fn collect_public_market(
    ...
    for adapter in adapters {
        ...
        let funding_by_symbol =
            match timeout(timeout_duration, adapter.load_funding(&symbols)).await {
                Ok(Ok(funding)) => {
...
fn current_positions_from_opportunities(
    opportunities: &[OpportunityReadModel],
    ...
    opportunities
        ...
        .take(50)
        .map(|opportunity| {
            let estimated_fee_usdt = opportunity.open_fee_est_usdt + opportunity.close_fee_est_usdt;
            let estimated_net_pnl_usdt =
                opportunity.maker_taker_net_edge * opportunity.target_notional_usdt;
            ShadowPositionReadModel {
                canonical_symbol: opportunity.canonical_symbol.clone(),
                long_exchange: opportunity.long_exchange.clone(),
                short_exchange: opportunity.short_exchange.clone(),
                notional_usdt: opportunity.target_notional_usdt,
                entry_edge_pct: opportunity.maker_taker_net_edge,
                expected_fee_usdt: estimated_fee_usdt,
                expected_funding_usdt: opportunity.expected_funding_usdt,
                estimated_net_pnl_usdt,
                status: "影子候选".to_string(),
                updated_at: now,
            }
        })
        .collect()
...
fn exchange_analysis_from_fills(fills: &[SimulatedMarketFill]) -> Vec<ExchangeAnalysisReadModel> {
    ...
    for fill in fills {
        ...
        long.notional += fill.target_notional_usdt;
        ...
        short.notional += fill.target_notional_usdt;
...
fn fee_analysis_from_fills(fills: &[SimulatedMarketFill]) -> Vec<FeeAnalysisReadModel> {
    ...
    for fill in fills {
        ...
        let long = by_exchange.entry(fill.long_exchange.clone()).or_default();
        long.0 += fee_half;
        long.1 += fill.target_notional_usdt;
        let short = by_exchange.entry(fill.short_exchange.clone()).or_default();
        short.0 += fee_half;
        short.1 += fill.target_notional_usdt;
    }
    let mut rows = by_exchange
        .into_iter()
...
<body>
  ...
  <main>
    ...
    <section class="layout">
      <div class="panel">
        <div class="title"><span>当前影子持仓候选</span><span class="sub">真实信号、真实盘口、真实资金费率，不实际下单</span></div>
    ...
    <section class="layout">
      <div class="panel">
        <div class="title"><span>历史影子仓位</span><span class="sub">市价吃单成交推演，按盘口和费用记录 PnL</span></div>
    ...
    <section class="tri">
      ...
      <div class="panel">
        <div class="title"><span>手续费分析</span><span class="sub">按交易所汇总预估费用</span></div>
  ...
  <script>
    ...
    function positions(rows) {
      table('positions', ['交易对','做多交易所','做空交易所','名义金额','入场净边际','开平手续费','资金费率','预计净 PnL','状态'],
        rows.slice(0, 100).map(r => [
          r.canonical_symbol, r.long_exchange, r.short_exchange, money.format(r.notional_usdt),
          pct(r.entry_edge_pct), money.format(r.expected_fee_usdt), pnlCell(r.expected_funding_usdt),
          pnlCell(r.estimated_net_pnl_usdt), `<span class="pill">${r.status}</span>`
        ]));
    }

    function symbols(rows) {
      table('symbols', ['交易对','最佳做多','最佳做空','原始价差','净边际','状态','拦截原因'],
    ...
    function fills(rows) {
      table('fills', ['交易对','做多','做空','名义金额','净 PnL','价差 PnL','手续费','资金费率','滑点成本','成交模型'],
        rows.slice(0, 160).map(r => [
          r.canonical_symbol, r.long_exchange, r.short_exchange, money.format(r.target_notional_usdt),
          pnlCell(r.net_pnl_usdt), money.format(r.gross_spread_pnl_usdt), money.format(r.fee_pnl_usdt),
          pnlCell(r.funding_pnl_usdt), money.format(r.slippage_cost_usdt), r.fill_model
        ]));
    }
    ...
    function pnl(a) {
      table('pnl', ['项目','数值'], [
        ...
        ['滑点成本', money.format(a.slippage_cost_usdt)],
    ...
    function opportunities(rows) {
      table('opps', ['交易对','做多','做空','Maker','Taker','原始价差','净边际','手续费','资金费率','盘口深度','盘口年龄'],
...
mod tests {
    ...
    fn cross_arb_server_router_should_build() {
        let state = AppState::new("missing-cross-arb-config.yml", 100, DataSource::Synthetic);
    ...
    fn cross_arb_server_should_seed_one_hundred_symbols() {
        ...
        assert!(read_model
            .simulated_fills
            .iter()
            .all(|fill| fill.fill_model == "market_all_filled"));
    }
    ...
    fn cross_arb_server_dashboard_should_force_simulation_without_live_orders() {
        ...
        let state = AppState::new(&config_path, 4, DataSource::Synthetic);
        let read_model = state.read_model.blocking_read();
