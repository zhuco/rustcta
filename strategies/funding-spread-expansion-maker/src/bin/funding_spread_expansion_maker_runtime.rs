...
use rustcta_strategy_funding_spread_expansion_maker::{
    FundingSpreadExpansionMakerRuntime, STRATEGY_KIND,
};
...
fn parse_args() -> Result<Args> {
    ...
    let mut args = Args {
        ...
        strategy_id: "funding_spread_expansion_maker_live".to_string(),
    ...
    while let Some(arg) = values.next() {
        match arg.as_str() {
            ...
            "--once" => args.once = true,
            "--help" | "-h" => {
                println!("funding-spread-expansion-maker-runtime --config <path> [--once]");
                std::process::exit(0);
            }
...
async fn emit_report(args: &Args, runtime: &FundingSpreadExpansionMakerRuntime) -> Result<()> {
