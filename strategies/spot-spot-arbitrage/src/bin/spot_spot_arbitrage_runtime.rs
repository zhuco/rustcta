...
use rustcta_strategy_spot_spot_arbitrage::{SpotSpotArbitrageRuntime, STRATEGY_KIND};
use serde::Serialize;
use serde_json::json;
...
fn parse_args() -> Result<Args> {
    ...
    let mut args = Args {
        config: PathBuf::from("config/spot_spot_arbitrage_live_dry_run_2ex_5symbols.yml"),
        strategy_id: "spot_spot_live_dry_run".to_string(),
    ...
    while let Some(arg) = values.next() {
        match arg.as_str() {
            ...
            "--once" => args.once = true,
            "--help" | "-h" => {
                println!("spot-spot-arbitrage-runtime --config <path> [--once]");
                std::process::exit(0);
            }
...
async fn emit_report(args: &Args, runtime: &SpotSpotArbitrageRuntime) -> Result<()> {
