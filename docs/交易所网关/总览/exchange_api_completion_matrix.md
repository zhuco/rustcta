...
Spot baseline is `ExchangeClient` in `retired exchange tree/unified.rs`:
...
`ExchangeClient` Spot contract exported from `retired exchange tree/mod.rs`: Binance,
...
counted in the legacy `retired exchange tree` Spot or `private_perp` percentages until
...
| --- | --- | --- | --- |
| BitMart | `retired exchange tree/bitmart/mod.rs` | Legacy `Exchange` | Broad mixed Spot/Futures implementation with balances, books, order submit/cancel/query/open/history, trades, fees, positions, leverage, cancel-all, batch/plan-order handling, symbol discovery, and websocket client construction. It is not wired to `ExchangeClient`, `TradingAdapter`, or the side-effect-free `private_perp` protocol, so Binance-parity completion is not comparable yet. Migration should start by splitting Spot and Futures surfaces and adding offline request-spec tests before claiming unified coverage. |
| Hyperliquid | `retired exchange tree/hyperliquid/mod.rs` | Legacy `Exchange` | Hyperliquid USDC perpetual adapter with custom signing, account/balance reads, ticker/book, order submit/cancel/query/open/history, trades, klines/statistics, fee readback, positions, leverage, composed cancel-all, batch orders, symbol discovery, and websocket client construction. It is outside the Binance USD-M CEX baseline and is not registered in the `private_perp` gateway path; track it as a venue-specific migration target instead of a missing Binance-parity row. |

## Binance Spot Changes In This Pass

Implemented and tested in `retired exchange tree/binance/spot.rs`:
...
Implemented and tested in `retired exchange tree/okx/spot.rs`:
...
Implemented and tested in `retired exchange tree/gateio/mod.rs`:
...
Implemented and tested in `retired exchange tree/coinex/mod.rs`:
...
Implemented and tested in `retired exchange tree/bitget/mod.rs`:
...
Implemented and tested in `retired exchange tree/paper/mod.rs`:
...
Implemented and tested in `retired exchange tree/mexc/mod.rs`:
...
Implemented and tested in `retired exchange tree/kucoin/mod.rs`:
...
Implemented and tested across `retired exchange tree/okx/core.rs`,
`retired exchange tree/private_perp/mod.rs`, `retired exchange tree/registry.rs`, and
`retired exchange tree/trading_adapters/mod.rs`:
...
Implemented and tested in `retired exchange tree/private_perp/mod.rs`:
...
`BinancePrivatePerpProtocol` in `retired exchange tree/private_perp/mod.rs`, while the
