...
Fiat payment methods, bank deposits, withdrawals, transfers, staking, ledger-writing funding operations, FIX, institutional account management, and non-Spot products are not connected to runtime. Amend order remains `Unsupported` because the shared request lacks Bitvavo `operatorId` semantics. The executable boundary fixture is `tests/fixtures/exchanges/bitvavo/unsupported_boundary.json`.
