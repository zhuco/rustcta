...
| 任务 | 主题 | 主要范围 | 必须协调 |
| --- | --- | --- | --- |
| 1 | 事件驱动行情与价差引擎 | `src/data`, `src/strategies/spot_spot_taker_arbitrage`, `strategies/spot-spot-arbitrage` | 2, 6, 7 |
| 2 | 严格 L2 订单簿与 resync | `src/data`, `src/exchanges`, `crates/rustcta-exchange-*` | 1, 6 |
| 3 | 双腿并发执行与单腿恢复 | `src/execution`, `crates/rustcta-execution-*` | 4, 5, 8 |
| 4 | 风控快路径 | `src/risk`, `src/execution`, `crates/rustcta-execution-*` | 3, 8 |
| 5 | 私有流优先订单状态 | `src/execution/user_stream.rs`, `src/exchanges`, `crates/rustcta-exchange-gateway` | 3, 6 |
| 6 | 端到端延迟指标 | `src/data`, `src/execution`, `src/market`, `crates/*` | 1, 2, 3, 5, 7 |
| 7 | 带延迟模型的回放 | `src/backtest`, `tests`, `data/replay` 相关 | 1, 6, 8 |
| 8 | 费用、库存、资金占用模型 | `src/execution/fee_model.rs`, `src/strategies/spot_spot_taker_arbitrage`, `src/control/spot_control` | 3, 4, 7 |
...
- `src/data/websocket_books.rs`
- `src/exchanges/**` 中的 public market-data parser/subscription
...
- `src/execution/order_reconciliation.rs`
- `src/exchanges/**` 中 private stream parser/subscription
- `crates/rustcta-exchange-api/**`
...
- `src/data/book_event.rs`
- `src/strategies/spot_spot_taker_arbitrage/replay.rs`
- `src/strategies/spot_spot_taker_arbitrage/report.rs`
