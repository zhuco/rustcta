...
`docs/exchange_adapter_toolchain_completion_zh.md`。当前 37 个已有 gateway
...
- 不把 CCXT 源码当作最终权威；CCXT 只用于能力发现，最终以官方 API 文档和本地
  fixture 为准。
- 不改策略层、执行层、控制面板、旧 `src/exchanges/*` 运行路径，除非任务明确是把
...
2. 阅读本文件、`docs/exchange_adapter_toolchain_completion_zh.md`、
...
```text
You are AI-06. Complete Task 6 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `krakenfutures` and `kucoinfutures`. Do not assume or specify a model. Reuse existing Kraken/KuCoin knowledge without changing their Spot semantics. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-08. Complete Task 8 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `paradex` and `derive`. Do not assume or specify a model. Keep DEX signing and options metadata explicit and secret-free. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-09. Complete Task 9 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `grvt` and `lighter`. Do not assume or specify a model. Favor conservative API maturity gates and robust offline fixtures. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-10. Complete Task 10 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `oxfun` and `pacifica`. Do not assume or specify a model. Verify official endpoints and testnet availability before private write support. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-11. Complete Task 11 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `aster` and `bitstamp`. Do not assume or specify a model. Prioritize Aster perpetual support, then Bitstamp spot completeness. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-13. Complete Task 13 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `bitflyer` and `bitbank`. Do not assume or specify a model. Keep Japanese market/product-code semantics explicit and avoid fiat funding operations. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-14. Complete Task 14 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `bitvavo` and `gemini`. Do not assume or specify a model. Implement spot trading gateway surfaces and keep fiat/funding operations out of runtime. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-17. Complete Task 17 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `btcmarkets` and `independentreserve`. Do not assume or specify a model. Implement spot gateway surfaces and document fiat/accounting boundaries. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-18. Complete Task 18 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `btcturk` and `luno`. Do not assume or specify a model. Keep regional availability and fiat-market behavior explicit. Do not run cargo build; run only the allowed checks and focused tests.
```
...
```text
You are AI-19. Complete Task 19 from docs/exchange_gateway_next_40_parallel_tasks_zh.md for `coinone` and `coinsph`. Do not assume or specify a model. Implement regional spot adapters with strict credential and funding boundaries. Do not run cargo build; run only the allowed checks and focused tests.
```
