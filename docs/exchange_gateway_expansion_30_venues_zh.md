...
| 65 | `novadax` | P2 | 巴西现货 | scan-only public REST，BRL 市场 | TODO |
...
- `docs/lbank_adapter.md`、`config/lbank_gateway_example.yml`、`tests/fixtures/exchanges/lbank/`：endpoint mapping、能力边界、配置示例和离线响应样本。
...
- `docs/woo_adapter.md`、`config/woo_gateway_example.yml`：endpoint mapping、签名说明、配置示例和能力边界。
...
- `docs/phemex_adapter.md`、`config/phemex_gateway_example.yml`：endpoint mapping、能力边界、配置示例和离线验证说明。
...
- `config/spot_exchanges_example.yml`、`config/cryptocom_gateway_example.yml`、`docs/cryptocom_adapter.md`、`tests/fixtures/exchanges/cryptocom/`：disabled-by-default 配置示例、endpoint mapping、认证规则、WS 心跳说明、能力边界和离线 fixture 样本。
...
- `src/exchanges/toobit/mod.rs`：Spot/USDT-M config、签名、REST transport、Spot public/private WS runtime、public/private parser、dry-run 和离线测试。
- `src/exchanges/market_adapters/toobit.rs`：Toobit USDT-M public market adapter，覆盖 contracts、funding、mark price、depth snapshot 和 public WS payload parser。
- `src/exchanges/private_perp/mod.rs`：Toobit USDT-M v2 futures order/batch/cancel/amend API request spec、`X-BB-APIKEY` query/body HMAC 签名、余额/仓位/成交/订单 parser、listenKey private WS endpoint/parser、private_perp adapter 注册与能力声明。
- `src/exchanges/mod.rs`：adapter module/export 注册。
- `src/exchanges/registry.rs`、`src/exchanges/trading_adapters/mod.rs`：Toobit private perpetual gateway/trading support 注册。
- `src/exchanges/client_order_id.rs`：Toobit Spot/Perpetual client order id policy。
...
- `docs/toobit_adapter.md`、`config/toobit_gateway_example.yml`：endpoint mapping、认证规则、配置示例和能力边界。
- `tests/fixtures/exchanges/toobit/`：exchange info、order book、balance、fills parser fixtures。
- `docs/exchange_api_completion_matrix.md`：capability 状态行。
...
- `docs/bingx_adapter.md`、`config/bingx_gateway_example.yml`、
...
- `config/bitunix_gateway_example.yml`、`docs/bitunix_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/poloniex_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `apps/gateway/src/config.rs`、`config/spot_exchanges_example.yml`、`docs/ascendex_adapter.md`：本地 gateway env wiring、默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `config/xt_gateway_example.yml`、`docs/xt_adapter.md`：默认 disabled-private-REST 配置示例、endpoint mapping、签名规则和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/tapbit_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/weex_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `docs/orangex_adapter.md`、`config/orangex_gateway_example.yml`：endpoint mapping、OAuth 签名规则、配置示例和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/coinw_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/bitrue_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `docs/deepcoin_adapter.md`、`config/deepcoin_gateway_example.yml`：endpoint mapping、认证规则、配置示例和能力边界。
...
- `config/hashkey_global_gateway_example.yml`、`docs/hashkey_global_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `docs/cointr_adapter.md`、`config/cointr_gateway_example.yml`：endpoint mapping、签名规则、配置示例和能力边界。
...
- `config/spot_exchanges_example.yml`、`docs/bigone_adapter.md`：disabled-by-default 配置示例、endpoint mapping、签名规则和能力边界。
...
- `docs/coindcx_adapter.md`、`config/coindcx_gateway_example.yml`：endpoint mapping、签名规则、Socket.IO 边界和配置示例。
...
代码边界：`crates/rustcta-exchange-gateway/src/adapters/bitkan/`；`crates/rustcta-exchange-gateway/src/adapters/mod.rs`、`crates/rustcta-exchange-gateway/src/lib.rs`、`apps/gateway/src/config.rs` 已接入；`docs/bitkan_adapter.md` 与 `config/bitkan_gateway_example.yml` 记录当前能力边界和升级门槛。
...
- `config/biconomy_gateway_example.yml`、`docs/biconomy_adapter.md`、`tests/fixtures/exchanges/biconomy/`：默认 disabled 配置示例、endpoint mapping、签名规则、能力边界和离线响应样本。
...
- `config/coinstore_gateway_example.yml`、`docs/coinstore_adapter.md`：默认 disabled 配置示例、endpoint mapping、签名规则和能力边界。
...
- `docs/digifinex_adapter.md`、`config/digifinex_gateway_example.yml`：endpoint mapping、签名规则、配置示例和能力边界。
...
- `完成 docs/exchange_gateway_expansion_30_venues_zh.md 中的开发任务 1`
- `完成 docs/exchange_gateway_expansion_30_venues_zh.md 中的开发任务 1-10`
- `完成 docs/exchange_gateway_expansion_30_venues_zh.md 中的开发任务 11-20`
- `完成 docs/exchange_gateway_expansion_30_venues_zh.md 中的开发任务 21-30`
