...
mod tests {
    use super::{
        expand_trend_factor_runs, PeriodGrid, TrendFactorEntryConfig, TrendFactorExitConfig,
        TrendFactorScanSpec, TrendFactorStrategyTemplate,
    };

    #[test]
    fn trend_factor_grid_expands_inside_backtest_crate() {
        let spec = TrendFactorScanSpec {
            ...
            cost_scenarios: Vec::new(),
            candidate_filter: Default::default(),
            strategies: vec![TrendFactorStrategyTemplate {
                name: "donchian".to_string(),
                entry: TrendFactorEntryConfig {
                    donchian_breakout: Some(PeriodGrid {
                        lookback: vec![20, 30],
                    }),
                    ..Default::default()
                },
        ...
        let runs = expand_trend_factor_runs(&spec).expect("grid should expand");
        assert_eq!(runs.len(), 4);
        assert_eq!(runs[0].run_id, 1);
        assert_eq!(runs[0].strategy_name, "donchian");
        assert!(runs[0]
            .parameters
            .contains_key("entry.donchian_breakout.lookback"));
        assert_eq!(runs[0].flags.get("exit.reverse_signal_exit"), Some(&true));
    }
}
