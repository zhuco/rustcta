...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("tapbit").expect("tapbit exchange")
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "BTC-SWAP")
            .expect("symbol"),
    }
}

pub(super) fn private_config(base_url: String) -> super::TapbitGatewayConfig {
    super::TapbitGatewayConfig {
        spot_rest_base_url: base_url.clone(),
        swap_rest_base_url: base_url,
        api_key: Some("key".to_string()),
        api_secret: Some("secret".to_string()),
        enabled_private_rest: true,
        ..super::TapbitGatewayConfig::default()
    }
}

pub(super) fn assert_signed_json_request(request: &SeenRequest, method: &str, path: &str) {
...
pub(super) fn request_spec(name: &str) -> crate::request_spec::RequestSpec {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/tapbit/request_specs/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("tapbit request spec fixture"))
        .expect("tapbit request spec json")
}

pub(super) fn signing_vector(name: &str) -> crate::signing_spec::SigningVector {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/tapbit/signing_vectors/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("tapbit signing vector fixture"))
        .expect("tapbit signing vector json")
}
