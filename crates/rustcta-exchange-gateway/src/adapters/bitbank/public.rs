...
use super::parser::{normalize_bitbank_pair, parse_orderbook_snapshot, parse_symbol_rules};
...
impl BitbankGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let value = self
            .rest
            .send_public_request("/v1/spot/pairs", &HashMap::new())
            .await?;
        ...
        if !request.symbols.is_empty() {
            let requested = request
                .symbols
                .iter()
                .map(|symbol| normalize_bitbank_pair(&symbol.exchange_symbol.symbol))
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let pair = normalize_bitbank_pair(&request.symbol.exchange_symbol.symbol)?;
        let value = self
            .rest
            .send_public_request(&format!("/{pair}/depth"), &HashMap::new())
            .await?;
