...
use super::parser::{
    normalize_depth, normalize_hollaex_symbol, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl HollaexGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let requested_symbols = request
            ...
            .map(|symbol| {
                ...
                normalize_hollaex_symbol(symbol)
            })
            .collect::<ExchangeApiResult<Vec<_>>>()?;

        let response = self
            .rest
            .send_public_get("/constants", &HashMap::new())
            .await?;
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let symbol = normalize_hollaex_symbol(&request.symbol)?;
        let depth = normalize_depth(request.depth);
        let mut params = HashMap::new();
        params.insert("symbol".to_string(), symbol.clone());
        let response = self.rest.send_public_get("/orderbook", &params).await?;
        let order_book =
            parse_orderbook_snapshot(&self.exchange_id, request.symbol, &symbol, depth, &response)?;
