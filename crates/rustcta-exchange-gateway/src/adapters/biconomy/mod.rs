...
pub use config::BiconomyGatewayConfig;
use transport::BiconomyRest;

#[derive(Clone)]
pub struct BiconomyGatewayAdapter {
    exchange_id: ExchangeId,
    config: BiconomyGatewayConfig,
    rest: BiconomyRest,
}

impl BiconomyGatewayAdapter {
    pub fn new(config: BiconomyGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("biconomy").map_err(validation_error)?;
        let rest = BiconomyRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BiconomyGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("biconomy adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    async fn send_signed_post(
        ...
        self.rest
            .send_signed_post(endpoint, body, api_key, api_secret, recv_window_ms)
            .await
    }
...
#[async_trait]
impl GatewayAdapter for BiconomyGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "biconomy spot public/private REST + WS request-spec gateway adapter"
                } else {
                    "biconomy spot public REST + WS request-spec gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for BiconomyGatewayAdapter {
    ...
    async fn place_quote_market_order(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "biconomy.quote_market_order",
        })
    }
    ...
    async fn amend_order(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "biconomy.amend_order",
        })
    }
