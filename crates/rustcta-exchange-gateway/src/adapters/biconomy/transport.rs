...
impl BiconomyRest {
    ...
    pub async fn send_signed_post(
        ...
        let signature = sign_payload(api_secret, timestamp, "POST", endpoint, &body_text)?;
        let response = self
            .http
            .post(format!(
                "{}{}",
...
async fn parse_response(
    ...
    if !status.is_success() {
        let message = error_message(&value).unwrap_or("Biconomy request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_biconomy_error(value.get("code").and_then(value_as_i64), message),
            message,
            Utc::now(),
        );
        error.code = value
    ...
    if code != 0 && code != 200 && code != 1000 {
        let message = error_message(&value).unwrap_or("Biconomy API error");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_biconomy_error(Some(code), message),
            message,
            Utc::now(),
        );
...
fn classify_biconomy_error(code: Option<i64>, message: &str) -> ExchangeErrorClass {
