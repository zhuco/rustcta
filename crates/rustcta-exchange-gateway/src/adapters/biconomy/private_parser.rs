...
use rustcta_types::{
    AccountId, AssetBalance, ExchangeId, FillStatus, LiquidityRole, MarketType, OrderSide,
    OrderStatus, OrderType, PositionSide, TenantId, TimeInForce,
};
...
pub fn parse_balances(
    ...
    let balances = rows
        .iter()
        .filter_map(|row| {
            let asset = text(row.get("coin").or_else(|| row.get("asset")))?;
            if !assets.is_empty()
                && !assets
                    .iter()
...
pub fn parse_fee_rates(
    ...
    Ok(symbols
        ...
        .map(|symbol| FeeRateSnapshot {
            ...
            source: Some("biconomy.private_get_account".to_string()),
            updated_at: Utc::now(),
        })
        .collect())
}
            ...
            Ok(Fill {
                ...
                canonical_symbol: symbol.canonical_symbol.clone().ok_or_else(|| {
                    ExchangeApiError::InvalidRequest {
                        message: "biconomy fill requires canonical_symbol".to_string(),
                    }
                })?,
                exchange_symbol: Some(symbol.exchange_symbol.clone()),
                order_id: text(row.get("orderId").or_else(|| row.get("id"))),
                client_order_id: text(row.get("clientOrderId")),
