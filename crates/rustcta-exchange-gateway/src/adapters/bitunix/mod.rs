...
pub use config::BitunixGatewayConfig;
use transport::BitunixRest;

#[derive(Clone)]
pub struct BitunixGatewayAdapter {
    exchange_id: ExchangeId,
    config: BitunixGatewayConfig,
    rest: BitunixRest,
}

impl BitunixGatewayAdapter {
    pub fn new(config: BitunixGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("bitunix").map_err(validation_error)?;
        let rest = BitunixRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BitunixGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("bitunix adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "bitunix.unsupported_market_type",
            });
        }
        Ok(())
    }
    ...
    async fn send_signed_get(
        ...
        self.rest
            .send_signed_get(market_type, endpoint, params, api_key, api_secret)
            .await
    }
    ...
    async fn send_signed_post(
        ...
        let (api_key, api_secret) = self.private_credentials(operation)?;
        self.rest
            .send_signed_post(market_type, endpoint, params, body, api_key, api_secret)
            .await
    }
...
#[async_trait]
impl GatewayAdapter for BitunixGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "bitunix spot + USDT perpetual public/private REST gateway adapter"
                } else {
                    "bitunix spot + USDT perpetual public REST gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for BitunixGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = if self.config.private_streams_enabled() {
            Some(streams::bitunix_private_stream_capabilities())
        } else {
    ...
    async fn place_quote_market_order(
        ...
        if request.symbol.market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "bitunix.perp_quote_market_order",
            });
        }
        if request.side != rustcta_types::OrderSide::Buy {
            return Err(ExchangeApiError::Unsupported {
                operation: "bitunix.spot_sell_quote_market_order",
            });
        }
    ...
    async fn place_order_list(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "bitunix.place_order_list",
        })
    }
