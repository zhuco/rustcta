...
fn parse_symbol_rule(exchange_id: &ExchangeId, value: &Value) -> ExchangeApiResult<SymbolRules> {
    ...
    let price_increment = string_or_number(value.get("priceIncrement"));
    let quantity_increment = string_or_number(value.get("baseIncrement"));
    let tradable = value
        .get("enableTrading")
    ...
    Ok(SymbolRules {
        ...
        min_price: None,
        max_price: None,
        min_quantity: string_or_number(value.get("baseMinSize")),
        max_quantity: string_or_number(value.get("baseMaxSize")),
        min_notional: string_or_number(value.get("quoteMinSize")),
        max_notional: string_or_number(value.get("quoteMaxSize")),
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "kucoin order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_kucoin_symbol(symbol: &str) -> ExchangeApiResult<String> {
    ...
    split_compact_symbol(&upper).ok_or_else(|| ExchangeApiError::InvalidRequest {
        message: format!("cannot infer KuCoin symbol from {symbol}"),
    })
}
