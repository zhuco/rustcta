...
pub use config::KuCoinGatewayConfig;
use transport::KuCoinPublicRest;

#[derive(Clone)]
pub struct KuCoinGatewayAdapter {
    exchange_id: ExchangeId,
    config: KuCoinGatewayConfig,
    rest: KuCoinPublicRest,
}

impl KuCoinGatewayAdapter {
    pub fn new(config: KuCoinGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("kucoin").map_err(validation_error)?;
        let rest = KuCoinPublicRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(KuCoinGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("kucoin adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "kucoin.non_spot_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "kucoin private REST readback requires context.tenant_id".to_string(),
                })?;
        let account_id =
            context
                .account_id
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "kucoin private REST readback requires context.account_id".to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for KuCoinGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("kucoin spot public REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for KuCoinGatewayAdapter {
    ...
    async fn get_positions(
        ...
        self.unsupported_private("kucoin.get_positions")
    }
    ...
    async fn place_order(
        ...
        self.unsupported_private("kucoin.place_order")
    }
    ...
    async fn cancel_order(
        ...
        self.unsupported_private("kucoin.cancel_order")
    }
