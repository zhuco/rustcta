...
pub use config::BlockchainComGatewayConfig;
use transport::BlockchainComRest;

#[derive(Clone)]
pub struct BlockchainComGatewayAdapter {
    exchange_id: ExchangeId,
    config: BlockchainComGatewayConfig,
    rest: BlockchainComRest,
}

impl BlockchainComGatewayAdapter {
    pub fn new(config: BlockchainComGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("blockchaincom").map_err(validation_error)?;
        let rest = BlockchainComRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "blockchaincom adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "blockchaincom.non_spot_market_type",
            });
        }
        Ok(())
    }
    ...
    fn ensure_public_rest(&self) -> ExchangeApiResult<()> {
        if !self.config.enabled_public_rest {
            return Err(ExchangeApiError::Unsupported {
                operation: "blockchaincom.public_rest_disabled",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for BlockchainComGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "blockchaincom spot public REST adapter with offline private request specs"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for BlockchainComGatewayAdapter {
    fn exchange(&self) -> ExchangeId {
        self.exchange_id.clone()
    }

    fn capabilities(&self) -> ExchangeClientCapabilities {
        let private_streams_enabled = self.config.private_stream_enabled();
        ...
        capabilities.private_stream_capabilities = Some(
            streams::blockchaincom_private_stream_capabilities(private_streams_enabled),
        );
    ...
    async fn get_positions(
        ...
        self.unsupported("blockchaincom.positions_unsupported_for_spot")
    }
    ...
    async fn place_order(
        ...
        self.unsupported("blockchaincom.place_order_offline_request_spec_only")
    }
    ...
    async fn place_quote_market_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_spot(request.symbol.market_type)?;
        self.unsupported("blockchaincom.quote_market_order_unverified")
    }
    ...
    async fn cancel_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_spot(request.symbol.market_type)?;
        self.unsupported("blockchaincom.cancel_order_offline_request_spec_only")
    }
    ...
    async fn amend_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_spot(request.symbol.market_type)?;
        self.unsupported("blockchaincom.amend_order_unsupported")
    }
    ...
    async fn place_order_list(
        ...
        self.ensure_exchange(&request.symbol().exchange)?;
        self.ensure_spot(request.symbol().market_type)?;
        self.unsupported("blockchaincom.order_list_unsupported")
    }
    ...
    async fn batch_place_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("blockchaincom.batch_place_orders_unsupported")
    }
    ...
    async fn batch_cancel_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("blockchaincom.batch_cancel_orders_unsupported")
    }
    ...
    async fn cancel_all_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("blockchaincom.cancel_all_orders_offline_request_spec_only")
    }
