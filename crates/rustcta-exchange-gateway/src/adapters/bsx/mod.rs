...
pub use config::BsxGatewayConfig;
use transport::BsxRest;

#[derive(Clone)]
pub struct BsxGatewayAdapter {
    exchange_id: ExchangeId,
    config: BsxGatewayConfig,
    rest: BsxRest,
}

impl BsxGatewayAdapter {
    pub fn new(config: BsxGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("bsx").map_err(validation_error)?;
        let rest = BsxRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("bsx adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "bsx.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for BsxGatewayAdapter {
...
#[async_trait]
impl ExchangeClient for BsxGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = Some(streams::bsx_private_stream_capabilities());
