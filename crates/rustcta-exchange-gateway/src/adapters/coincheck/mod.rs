...
pub use config::CoincheckGatewayConfig;
use transport::CoincheckPublicRest;

#[derive(Clone)]
pub struct CoincheckGatewayAdapter {
    exchange_id: ExchangeId,
    config: CoincheckGatewayConfig,
    rest: CoincheckPublicRest,
}

impl CoincheckGatewayAdapter {
    pub fn new(config: CoincheckGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("coincheck").map_err(validation_error)?;
        let rest = CoincheckPublicRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(CoincheckGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("coincheck adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "coincheck.non_spot_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "coincheck private REST readback requires context.tenant_id"
                        .to_string(),
                })?;
        let account_id =
            context
                ...
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "coincheck private REST readback requires context.account_id"
                        .to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for CoincheckGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("coincheck spot public REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for CoincheckGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities =
            Some(streams::coincheck_private_stream_capabilities(false));
        ...
        capabilities.capabilities_v2.private_streams =
            CapabilitySupport::unsupported("coincheck has no adapter-verified private websocket");
        capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
            ...
            private: CapabilitySupport::rest_fallback(
                "coincheck private order/fill synchronization uses REST reconciliation",
            ),
        ...
        capabilities.capabilities_v2.batch_place_orders = BatchCapability {
            support: if self.config.private_rest_enabled() {
                CapabilitySupport::composed("Coincheck has no native batch place endpoint; gateway sends sequential order creates")
            } else {
                CapabilitySupport::unsupported(
                    "coincheck batch place requires private REST credentials",
                )
            },
        ...
        capabilities.capabilities_v2.batch_cancel_orders = BatchCapability {
            support: if self.config.private_rest_enabled() {
                CapabilitySupport::composed(
                    "Coincheck batch cancel is gateway-composed from sequential cancel requests",
                )
            } else {
                CapabilitySupport::unsupported(
                    "coincheck batch cancel requires private REST credentials",
                )
            },
            mode: if self.config.private_rest_enabled() {
                BatchExecutionMode::ComposedSequential
        ...
        capabilities.capabilities_v2.fills_history = HistoryCapability {
            support: if self.config.private_rest_enabled() {
                ...
                CapabilitySupport::unsupported(
                    "coincheck fills history requires private REST credentials",
                )
            },
        ...
        capabilities.capabilities_v2.order_history = HistoryCapability {
            support: CapabilitySupport::unsupported(
                "coincheck single order history is not mapped; use open orders and transaction reconciliation",
            ),
            supports_since: false,
            supports_until: false,
            supports_limit: true,
    ...
    async fn get_positions(
        ...
        self.unsupported_private("coincheck.get_positions")
    }
