...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("htx").expect("exchange")
}
...
pub(super) fn spot_symbol() -> SymbolScope {
    SymbolScope {
...
pub(super) fn perp_symbol() -> SymbolScope {
    SymbolScope {
        exchange: exchange_id(),
        market_type: MarketType::Perpetual,
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "BTC-USDT")
            .expect("symbol"),
    }
}
...
pub(super) fn load_fixture(path: &str) -> Value {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/htx/{path}",
        env!("CARGO_MANIFEST_DIR")
    );
    let text = std::fs::read_to_string(path).expect("htx fixture");
    serde_json::from_str(&text).expect("htx fixture json")
}
...
pub(super) fn load_request_spec(path: &str) -> crate::request_spec::RequestSpec {
    ...
    let text = std::fs::read_to_string(path).expect("htx request spec fixture");
    serde_json::from_str(&text).expect("htx request spec fixture")
}
