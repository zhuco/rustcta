...
pub use config::HtxGatewayConfig;
use transport::{HtxRest, HtxRestProduct};

#[derive(Clone)]
pub struct HtxGatewayAdapter {
    exchange_id: ExchangeId,
    config: HtxGatewayConfig,
    rest: HtxRest,
}

impl HtxGatewayAdapter {
    pub fn new(config: HtxGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new(config.profile.adapter_id()).map_err(validation_error)?;
        let rest = HtxRest::new(
            exchange_id.clone(),
            config.spot_rest_base_url.clone(),
            config.linear_rest_base_url.clone(),
    ...
    pub fn huobi_legacy() -> ExchangeApiResult<Self> {
        Self::new(HtxGatewayConfig::huobi_legacy())
    }

    #[cfg(test)]
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(HtxGatewayConfig::default())
    }
        ...
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    ...
                    self.config.profile.adapter_id()
                ),
            });
        }
        Ok(())
    ...
    async fn send_signed_post(
        ...
        self.rest
            .send_signed_post(product, endpoint, body, api_key, api_secret)
            .await
    }
    ...
    async fn send_signed_get(
        ...
        let (api_key, api_secret) = self.private_credentials(operation)?;
        self.rest
            .send_signed_get(product, endpoint, params, api_key, api_secret)
            .await
    }
...
#[async_trait]
impl GatewayAdapter for HtxGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(format!(
                "{} spot + usdt perpetual REST and WS request-spec gateway adapter",
...
#[async_trait]
impl ExchangeClient for HtxGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.order_book =
            rustcta_exchange_api::OrderBookCapability::best_effort_delta(Some(150));
        capabilities.max_recent_fill_limit = Some(50);
        capabilities
    }
    ...
    async fn place_quote_market_order(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "htx.quote_market_order",
        })
    }
