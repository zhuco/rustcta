...
pub use config::KuCoinFuturesGatewayConfig;
use transport::KuCoinFuturesRest;

#[derive(Clone)]
pub struct KuCoinFuturesGatewayAdapter {
    exchange_id: ExchangeId,
    config: KuCoinFuturesGatewayConfig,
    rest: KuCoinFuturesRest,
}

impl KuCoinFuturesGatewayAdapter {
    pub fn new(config: KuCoinFuturesGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("kucoinfutures").map_err(validation_error)?;
        let rest = KuCoinFuturesRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(KuCoinFuturesGatewayConfig::default())
    }
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "kucoinfutures adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_perpetual(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "kucoinfutures.non_perpetual_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "kucoinfutures private REST readback requires context.tenant_id"
                        .to_string(),
                })?;
        let account_id =
            context
                ...
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "kucoinfutures private REST readback requires context.account_id"
                        .to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for KuCoinFuturesGatewayAdapter {
...
#[async_trait]
impl ExchangeClient for KuCoinFuturesGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = Some(
            streams::kucoinfutures_private_stream_capabilities(self.config.private_rest_enabled()),
        );
        ...
        capabilities.capabilities_v2.private_streams = if self.config.private_rest_enabled() {
            ...
            CapabilitySupport::unsupported(
                "kucoinfutures private streams require bullet-private token",
            )
        };
        ...
        capabilities.capabilities_v2.batch_place_orders = BatchCapability {
            support: if self.config.private_rest_enabled() {
                CapabilitySupport::composed("kucoinfutures has no adapter-declared native batch place endpoint; gateway sends sequential hf order creates")
            } else {
                CapabilitySupport::unsupported(
                    "kucoinfutures batch place requires private REST credentials",
                )
            },
        ...
        capabilities.capabilities_v2.batch_cancel_orders = BatchCapability {
            support: if self.config.private_rest_enabled() {
                CapabilitySupport::composed(
                    "kucoinfutures batch cancel is gateway-composed from sequential hf cancel requests",
                )
            } else {
                CapabilitySupport::unsupported(
                    "kucoinfutures batch cancel requires private REST credentials",
                )
            },
            mode: if self.config.private_rest_enabled() {
                BatchExecutionMode::ComposedSequential
        ...
        capabilities.capabilities_v2.fills_history = HistoryCapability {
            support: if self.config.private_rest_enabled() {
                ...
                CapabilitySupport::unsupported(
                    "kucoinfutures fills history requires private REST credentials",
                )
            },
            supports_since: true,
            supports_until: true,
        ...
        capabilities.capabilities_v2.order_history = HistoryCapability {
            support: if self.config.private_rest_enabled() {
                ...
                CapabilitySupport::unsupported(
                    "kucoinfutures order history requires private REST credentials",
                )
            },
