...
use super::parser::{
    decimal_value_to_f64, first_timestamp_millis, normalize_kucoinfutures_symbol, parse_error,
...
pub fn parse_account_balances(
    ...
    let accounts = if accounts.len() == 1 && accounts[0].is_null() {
    ...
    for account in accounts {
        let asset = account
            .get("currency")
            .or_else(|| account.get("accountEquityCurrency"))
        ...
        let available = decimal_value_to_f64(
            account
                ...
                .or_else(|| account.get("available")),
        )?
        .unwrap_or(0.0);
        let total = decimal_value_to_f64(
            account
                .get("accountEquity")
                .or_else(|| account.get("marginBalance"))
...
pub fn parse_positions(
    ...
    for row in rows {
        let symbol_text = row
            ...
            .unwrap_or_default()
            .to_ascii_uppercase();
        if symbol_text.is_empty() || (!wanted.is_empty() && !wanted.contains(&symbol_text)) {
            continue;
        }
        ...
        let (base, quote) = split_futures_symbol(&symbol_text);
        ...
        let position = ExchangePosition {
            ...
            entry_price: decimal_value_to_f64(
                row.get("avgEntryPrice").or_else(|| row.get("entryPrice")),
            )?,
            ...
            leverage: decimal_value_to_f64(
                row.get("realLeverage").or_else(|| row.get("leverage")),
            )?,
            observed_at: Utc::now(),
        };
        ...
        .map(|item| {
            ...
            Ok(FeeRateSnapshot {
                ...
                taker_rate: string_or_number(item.get("takerFeeRate"))
                    ...
                    .unwrap_or_else(|| "0".to_string()),
                source: Some("kucoinfutures.trade_fees".to_string()),
                updated_at: Utc::now(),
            })
        })
        .collect()
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "kucoinfutures recent fills request requires canonical_symbol".to_string(),
            })?;
    let items = items_array(value);
    items
        .iter()
