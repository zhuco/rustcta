...
impl SeenRequest {
    pub(super) fn to_actual_request(&self) -> crate::request_spec::ActualHttpRequest {
        crate::request_spec::ActualHttpRequest {
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("cointr").expect("exchange")
}
...
pub(super) fn assert_private_request(request: &SeenRequest, path: &str, api_key: &str) {
    ...
    assert!(request.headers.get("access-timestamp").is_some());
    assert!(request.headers.get("access-passphrase").is_some());
}

pub(super) fn load_fixture(path: &str) -> Value {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/cointr/{path}",
        env!("CARGO_MANIFEST_DIR")
    );
    let text = std::fs::read_to_string(path).expect("cointr fixture");
    serde_json::from_str(&text).expect("cointr fixture json")
}

pub(super) fn load_request_spec(path: &str) -> crate::request_spec::RequestSpec {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/cointr/{path}",
        env!("CARGO_MANIFEST_DIR")
    );
    let text = std::fs::read_to_string(path).expect("cointr request spec fixture");
    serde_json::from_str(&text).expect("cointr request spec fixture")
}
