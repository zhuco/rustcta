...
pub use config::CointrGatewayConfig;
use transport::CointrRest;

#[derive(Clone)]
pub struct CointrGatewayAdapter {
    exchange_id: ExchangeId,
    config: CointrGatewayConfig,
    rest: CointrRest,
}

impl CointrGatewayAdapter {
    pub fn new(config: CointrGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("cointr").map_err(validation_error)?;
        let rest = CointrRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(CointrGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("cointr adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "cointr.unsupported_market_type",
            });
        }
        Ok(())
    }
    ...
    fn private_credentials(
    ...
    ) -> ExchangeApiResult<(&str, &str, &str, u64)> {
        ...
        let passphrase = self
            ...
            .filter(|value| !value.trim().is_empty())
            .ok_or(ExchangeApiError::Unsupported { operation })?;
        Ok((api_key, api_secret, passphrase, self.config.recv_window_ms))
    }
    ...
    async fn send_signed_get(
        ...
        let (api_key, api_secret, passphrase, _) = self.private_credentials(operation)?;
        self.rest
    ...
    async fn send_signed_post(
        ...
        let (api_key, api_secret, passphrase, recv_window_ms) =
...
#[async_trait]
impl GatewayAdapter for CointrGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "cointr spot + usdt futures public/private REST + WS request-spec gateway adapter"
                } else {
                    "cointr spot + usdt futures public REST + WS request-spec gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for CointrGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = Some(PrivateStreamCapabilities {
            ...
            supports_client_order_id: private,
            supports_exchange_order_id: private,
        });
        ...
        toolchain::apply_cointr_toolchain_capabilities(&mut capabilities, private);
        capabilities
    }
    ...
    async fn place_quote_market_order(
        ...
        if request.symbol.market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "cointr.perp_quote_market_order",
            });
        }
        if request.side != OrderSide::Buy {
            return Err(ExchangeApiError::Unsupported {
                operation: "cointr.spot_sell_quote_market_order",
            });
        }
