...
pub fn parse_symbol_rules(
    ...
    rows(value)
        ...
        .map(|row| {
            let symbol_text =
                required_text(exchange_id, row, &["symbol", "symbolName", "baseCoin", "s"])?;
            ...
            Ok(SymbolRules {
                ...
                price_increment: text(
                    row.get("tickSize")
                        .or_else(|| row.get("priceTick"))
                        .or_else(|| row.get("pricePlace"))
                        .or_else(|| row.get("priceEndStep")),
                ),
                quantity_increment: text(
                    row.get("stepSize")
                        .or_else(|| row.get("quantityTick"))
                        .or_else(|| row.get("volumePlace"))
                        .or_else(|| row.get("sizeMultiplier")),
                ),
                min_price: text(row.get("minPrice")),
                max_price: text(row.get("maxPrice")),
                min_quantity: text(
                    row.get("minQty")
                        .or_else(|| row.get("minQuantity"))
                        .or_else(|| row.get("minTradeNum")),
                ),
                max_quantity: text(row.get("maxQty").or_else(|| row.get("maxQuantity"))),
                min_notional: text(row.get("minNotional").or_else(|| row.get("minTradeUSDT"))),
                max_notional: None,
...
pub fn parse_order_book(
    ...
    let bids = data
        ...
        .ok_or_else(|| parse_error(exchange_id.clone(), "Cointr order book missing bids", value))?;
    let asks = data
        .get("asks")
        .and_then(Value::as_array)
        .ok_or_else(|| parse_error(exchange_id.clone(), "Cointr order book missing asks", value))?;
    let canonical_symbol =
        symbol
            .canonical_symbol
            .clone()
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "cointr order book requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_symbol(symbol: &SymbolScope) -> ExchangeApiResult<String> {
    ...
    Err(ExchangeApiError::InvalidRequest {
        message: "cointr symbol requires exchange_symbol or canonical_symbol".to_string(),
    })
}
...
fn infer_assets(row: &Value, symbol: &str) -> (String, String) {
    let base = text(
        row.get("baseAsset")
            ...
            .or_else(|| row.get("baseCoin")),
    )
    .unwrap_or_else(|| split_symbol(symbol).0);
    let quote = text(
        row.get("quoteAsset")
            .or_else(|| row.get("quote"))
            .or_else(|| row.get("quoteCoin"))
            .or_else(|| row.get("quoteAsset")),
    )
