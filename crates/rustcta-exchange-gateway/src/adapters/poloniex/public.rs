...
use super::parser::{
    normalize_depth, normalize_poloniex_symbol, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl PoloniexGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        for market_type in markets {
            let endpoint = match market_type {
                MarketType::Spot => "/markets",
                MarketType::Perpetual => "/v3/market/allInstruments",
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => {
                format!("/markets/{symbol}/orderBook")
            }
            MarketType::Perpetual => {
                params.insert("symbol".to_string(), symbol);
                "/v3/market/orderBook".to_string()
            }
            _ => unreachable!("checked by ensure_supported_market"),
        };
