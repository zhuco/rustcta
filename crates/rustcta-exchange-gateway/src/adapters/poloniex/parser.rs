...
fn parse_symbol_rule(
    ...
    let (fallback_base, fallback_quote) = split_poloniex_symbol(&exchange_symbol)
        .unwrap_or_else(|| ("UNKNOWN".to_string(), "USDT".to_string()));
    let base_asset = value
        .get("baseCurrencyName")
        .or_else(|| value.get("bCcy"))
    ...
    let tradable = !matches!(
        ...
        "HALT" | "SUSPEND" | "OFFLINE" | "CLOSE" | "DISABLED" | "PAUSE"
    );
    ...
    Ok(SymbolRules {
        ...
        base_asset,
        quote_asset,
        price_increment: string_or_number(value.get("tSz"))
            .or_else(|| string_or_number(trade_limit.get("tickSize")))
            .or_else(|| price_precision.map(increment_from_precision)),
        quantity_increment: string_or_number(value.get("minSz"))
            .or_else(|| string_or_number(trade_limit.get("stepSize")))
            .or_else(|| string_or_number(trade_limit.get("size")))
        ...
        min_quantity: string_or_number(
            value
                .get("minQuantity")
                .or_else(|| value.get("minSz"))
                .or_else(|| trade_limit.get("minQty"))
                .or_else(|| trade_limit.get("tradeMinLimit"))
                .or_else(|| trade_limit.get("minTradeQty")),
        ),
        max_quantity: string_or_number(
            trade_limit
                .get("maxQuantity")
                .or_else(|| trade_limit.get("maxQty"))
                .or_else(|| trade_limit.get("maxTradeQty")),
        ),
        min_notional: string_or_number(
            trade_limit
                .get("minAmount")
                .or_else(|| trade_limit.get("minNotional")),
        ),
        max_notional: string_or_number(trade_limit.get("maxAmount")),
        price_precision,
        quantity_precision,
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "poloniex order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_poloniex_symbol(
    symbol: &str,
    market_type: MarketType,
    ...
    let (base, quote) =
        split_compact_symbol(&compact).ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: format!("cannot normalize Poloniex symbol {symbol}"),
        })?;
    let suffix = if market_type == MarketType::Perpetual {
        "_PERP"
    } else {
        ""
    };
    Ok(format!("{base}_{quote}{suffix}"))
}
...
fn parse_levels(
    ...
    if levels.first().is_some_and(|level| !level.is_array()) {
        return levels
            .chunks(2)
            .map(|chunk| {
                let price = chunk.first().and_then(number_from_value).ok_or_else(|| {
                    parse_error(
                        exchange_id.clone(),
                        "invalid flat level price",
                        &Value::Null,
                    )
                })?;
                let quantity = chunk.get(1).and_then(number_from_value).ok_or_else(|| {
                    parse_error(
                        exchange_id.clone(),
                        "invalid flat level quantity",
                        &Value::Null,
                    )
                })?;
...
pub(super) fn split_poloniex_symbol(symbol: &str) -> Option<(String, String)> {
    let symbol = symbol.trim_end_matches("_PERP");
    if let Some((base, quote)) = symbol.split_once('_').or_else(|| symbol.split_once('-')) {
