...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("poloniex").expect("poloniex exchange")
}
...
pub(super) fn assert_signed_poloniex_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert_eq!(
        request.headers.get("recvwindow").map(String::as_str),
        Some("5000")
    );
    assert!(request
        .headers
