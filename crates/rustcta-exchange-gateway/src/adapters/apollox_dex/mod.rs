...
pub use config::ApolloxDexGatewayConfig;
use transport::ApolloxDexRest;

#[derive(Clone)]
pub struct ApolloxDexGatewayAdapter {
    exchange_id: ExchangeId,
    config: ApolloxDexGatewayConfig,
    rest: ApolloxDexRest,
}

impl ApolloxDexGatewayAdapter {
    pub fn new(config: ApolloxDexGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("apollox_dex").map_err(validation_error)?;
        let rest = ApolloxDexRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "apollox_dex adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
...
#[async_trait]
impl GatewayAdapter for ApolloxDexGatewayAdapter {
...
#[async_trait]
impl ExchangeClient for ApolloxDexGatewayAdapter {
