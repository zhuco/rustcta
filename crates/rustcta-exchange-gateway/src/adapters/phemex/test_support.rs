...
pub(super) async fn spawn_rest_server(
    ...
    tokio::spawn(async move {
        loop {
            ...
            while !request_body_complete(&request_bytes) {
                let bytes_read = stream.read(&mut buffer).await.unwrap();
                if bytes_read == 0 {
                    break;
                }
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("phemex").expect("phemex exchange")
}
...
pub(super) fn spot_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "sBTCUSDT")
            .expect("symbol"),
    }
}
...
pub(super) fn assert_signed_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert_eq!(
        request
            .headers
            .get("x-phemex-access-token")
            .map(String::as_str),
        Some("key")
    );
    assert!(request
        .headers
        .get("x-phemex-request-expiry")
        .is_some_and(|value| !value.is_empty()));
    assert!(request
        .headers
        .get("x-phemex-request-signature")
        .is_some_and(|value| !value.is_empty()));
}
...
fn request_body_complete(request_bytes: &[u8]) -> bool {
