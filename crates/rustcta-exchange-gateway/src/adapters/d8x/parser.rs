...
pub(super) fn parse_position_source_boundary(
    ...
    require_text(exchange_id, value, "exchange", "d8x")?;
    ...
    let fixture_policy = value.get("fixture_policy").ok_or_else(|| {
        parse_error(
            exchange_id.clone(),
            "D8X position source boundary missing fixture_policy",
            value,
        )
    })?;
...
fn require_text(
    ...
    Err(parse_error(
        exchange_id.clone(),
        &format!("D8X source boundary {field} expected {expected}, got {actual}"),
        value,
    ))
}
...
fn require_bool(
    ...
    match value.get(field).and_then(Value::as_bool) {
        ...
        _ => Err(parse_error(
            exchange_id.clone(),
            &format!("D8X source boundary {field} expected {expected}"),
            value,
        )),
    }
}
...
fn required_string_array(
    ...
    for key in path {
        cursor = cursor.get(*key).ok_or_else(|| {
            parse_error(
                exchange_id.clone(),
                &format!("D8X source boundary missing {}", path.join(".")),
                value,
            )
        })?;
    }
    let values = cursor.as_array().ok_or_else(|| {
        parse_error(
            exchange_id.clone(),
            &format!("D8X source boundary {} must be an array", path.join(".")),
            cursor,
        )
    })?;
    let parsed = values
        ...
        .ok_or_else(|| {
            parse_error(
                exchange_id.clone(),
                &format!(
                    "D8X source boundary {} must contain strings",
                    path.join(".")
                ),
                cursor,
            )
    ...
    if parsed.is_empty() {
        return Err(parse_error(
            exchange_id.clone(),
            &format!("D8X source boundary {} must not be empty", path.join(".")),
            cursor,
        ));
    }
    Ok(parsed)
