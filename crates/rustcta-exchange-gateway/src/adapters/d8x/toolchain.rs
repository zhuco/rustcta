...
pub(super) fn apply_toolchain_capabilities(capabilities: &mut ExchangeClientCapabilities) {
    ...
    capabilities.capabilities_v2.private_rest = CapabilitySupport::unsupported(
        "D8X private reads and writes require audited EVM wallet and contract-call semantics",
    );
    capabilities.capabilities_v2.public_streams =
        CapabilitySupport::unsupported("D8X WS runtime is not verified; REST reconciliation first");
    capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
        "D8X private streams require wallet auth and contract/indexer audit",
    );
    capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
        public: CapabilitySupport::unsupported(
            "D8X public stream payloads are fixture-only in this adapter",
        ),
        private: CapabilitySupport::unsupported("D8X private WS auth is not mapped"),
...
fn endpoint_capabilities() -> Vec<EndpointCapability> {
    let mut endpoints = vec![
        EndpointCapability {
            ...
            path: Some("/coingecko/contracts".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("d8x_public".to_string()),
            weight: Some(1),
        ...
        EndpointCapability {
            ...
            transport: EndpointTransport::Rest,
            method: Some("GET".to_string()),
            path: Some("/coingecko/orderbook/{ticker_id}".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("d8x_public".to_string()),
            weight: Some(1),
    ...
    endpoints.extend(
        ...
        .map(|(operation, reason)| EndpointCapability {
            ...
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("d8x_unsupported".to_string()),
            weight: Some(0),
