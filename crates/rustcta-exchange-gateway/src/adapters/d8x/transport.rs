...
impl D8xRest {
    ...
    pub fn contracts_path(chain_id: u64) -> String {
        format!("/coingecko/contracts?chain_id={chain_id}")
    }

    pub fn orderbook_path(ticker_id: &str, chain_id: u64) -> String {
        format!(
            "/coingecko/orderbook/{}?chain_id={chain_id}",
            ticker_id.trim().to_ascii_lowercase()
        )
    }

    pub fn orderbook_spec_path(ticker_id: &str) -> String {
        format!(
            "/coingecko/orderbook/{}",
            ticker_id.trim().to_ascii_lowercase()
        )
