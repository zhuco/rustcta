...
pub use config::D8xGatewayConfig;
use transport::D8xRest;

#[derive(Clone)]
pub struct D8xGatewayAdapter {
    exchange_id: ExchangeId,
    config: D8xGatewayConfig,
    rest: D8xRest,
}

impl D8xGatewayAdapter {
    pub fn new(config: D8xGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("d8x").map_err(validation_error)?;
        let rest = D8xRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("d8x adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "d8x.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for D8xGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(format!(
                "d8x {} perpetual profile: public market-data REST enabled; contract writes Unsupported",
                self.config.chain_profile
            )),
        }
    }
}
...
#[async_trait]
impl ExchangeClient for D8xGatewayAdapter {
