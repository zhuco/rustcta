...
use rustcta_exchange_api::{
    ...
    OrderListResponse, ResponseMetadata, SymbolScope, TenantId, EXCHANGE_API_SCHEMA_VERSION,
};
use rustcta_types::{
...
pub fn parse_account_balances(
    ...
    let rows = order_rows(value).ok_or_else(|| ExchangeApiError::InvalidRequest {
        message: "HitBTC balances response missing balance array".to_string(),
    })?;
    ...
    for row in rows {
        let asset = row
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "HitBTC balance row missing currency".to_string(),
            })?
            .to_ascii_uppercase();
...
pub fn parse_fee_snapshots(
    ...
    let rows = order_rows(value).ok_or_else(|| ExchangeApiError::InvalidRequest {
        message: "HitBTC fee response missing fee array".to_string(),
    })?;
    ...
    for row in rows {
        ...
        fees.push(FeeRateSnapshot {
            ...
            taker_rate: string_or_number(row.get("take_rate").or_else(|| row.get("taker_rate")))
                .unwrap_or_else(|| "0".to_string()),
            source: Some("hitbtc.spot_fee".to_string()),
            updated_at: Utc::now(),
        });
    }
    Ok(fees)
...
pub fn parse_order_list_ack(
    ...
    let orders = value
        .get("orders")
        .or_else(|| value.get("order_reports"))
        .and_then(Value::as_array)
        .ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: "HitBTC order-list response missing orders".to_string(),
        })?
        .iter()
        .map(|order| parse_spot_order_with_symbol_hint(exchange_id, &symbol, order))
    ...
    Ok(OrderListResponse {
        ...
        symbol,
        kind,
        order_list_id: string_or_number(value.get("id").or_else(|| value.get("order_list_id"))),
        list_client_order_id: value
            .get("client_order_id")
            .or_else(|| value.get("list_client_order_id"))
            .and_then(Value::as_str)
            .map(str::to_string),
...
pub fn parse_batch_cancel_orders_ack(
    ...
    let orders = value
        ...
        .or_else(|| value.get("order_reports").and_then(Value::as_array))
        .ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: "HitBTC batch-cancel response missing order array".to_string(),
        })?;

    orders
        .iter()
        .map(|order| {
            if order.get("error").is_some() {
                return Err(ExchangeApiError::InvalidRequest {
                    message: format!("HitBTC batch-cancel item rejected: {order}"),
                });
            }
            parse_spot_order_with_symbol_hint(exchange_id, &symbol, order)
        })
        .collect()
}
...
pub fn parse_open_orders(
    ...
    let orders = order_rows(value).ok_or_else(|| ExchangeApiError::InvalidRequest {
        message: "HitBTC open orders response missing order array".to_string(),
    })?;
    orders
        .iter()
...
pub fn parse_recent_fills(
    ...
    let canonical_symbol =
        symbol
            ...
            .clone()
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "HitBTC recent fills require canonical_symbol".to_string(),
            })?;
    let trades = order_rows(value).ok_or_else(|| ExchangeApiError::InvalidRequest {
        message: "HitBTC trade history response missing trade array".to_string(),
    })?;
    trades
        .iter()
        .map(|trade| {
            let price = number_from_value_or_zero(trade.get("price"));
            let quantity = number_from_value_or_zero(
                trade
                    .get("quantity")
...
fn parse_spot_order_with_symbol_hint(
    exchange_id: &ExchangeId,
    symbol: &SymbolScope,
    value: &Value,
) -> ExchangeApiResult<rustcta_exchange_api::OrderState> {
    let symbol = value
        .get("symbol")
        .and_then(Value::as_str)
        .map(|exchange_symbol| symbol_scope_from_exchange_symbol(exchange_id, exchange_symbol))
        .transpose()?
    ...
    let symbol = if let Some(symbol) = symbol_hint {
        ...
        let exchange_symbol = value.get("symbol").and_then(Value::as_str).ok_or_else(|| {
            ExchangeApiError::InvalidRequest {
                message: "HitBTC order response missing symbol".to_string(),
            }
        })?;
...
fn fee_symbol(
    ...
    Err(ExchangeApiError::InvalidRequest {
        message: "HitBTC fee row missing symbol".to_string(),
    })
}
...
fn number_from_value_or_zero(value: Option<&Value>) -> f64 {
    value.and_then(number_from_value).unwrap_or(0.0)
}
...
fn parse_side(side: Option<&str>) -> ExchangeApiResult<OrderSide> {
    match side.unwrap_or_default() {
        ...
        other => Err(ExchangeApiError::InvalidRequest {
            message: format!("unknown HitBTC fill side {other}"),
        }),
    }
}
