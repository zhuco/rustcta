...
pub fn sign_hs256_hex(secret: &str, payload: &str) -> ExchangeApiResult<String> {
    let mut mac = HmacSha256::new_from_slice(secret.as_bytes()).map_err(|error| {
        ExchangeApiError::InvalidRequest {
            message: format!("invalid HitBTC API secret length: {error}"),
        }
    })?;
...
mod tests {
    ...
    #[test]
    fn hitbtc_signing_should_match_fixed_rest_vector() {
        let payload = rest_hs256_payload(
            "POST",
            "/api/3/spot/order",
            None,
            Some("client_order_id=cli-hitbtc-1&price=0.046016&quantity=0.063&side=sell&symbol=ETHBTC&type=limit"),
            1_710_000_000_000,
            Some(10_000),
        );
        assert_eq!(
            payload,
            "POST/api/3/spot/orderclient_order_id=cli-hitbtc-1&price=0.046016&quantity=0.063&side=sell&symbol=ETHBTC&type=limit171000000000010000"
        );
        assert_eq!(
            sign_hs256_hex("test-secret", &payload).expect("signature"),
            "8935e293c3e2752fc4684fc648c9f9435e7fe5259cbf8583991e0fa64713cbac"
        );
    }

    #[test]
    fn hitbtc_signing_should_match_fixed_ws_vector() {
    ...
    #[test]
    fn hitbtc_authorization_header_should_not_expose_plain_secret() {
