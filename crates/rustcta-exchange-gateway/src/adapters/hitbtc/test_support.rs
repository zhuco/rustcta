...
pub(super) async fn spawn_rest_server(
    ...
    (format!("http://{address}/api/3"), seen)
}
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("hitbtc").expect("hitbtc exchange")
}
...
pub(super) fn symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("ETH", "BTC").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "ETHBTC")
            .expect("symbol"),
    }
}

pub(super) fn config_with_base_url(rest_base_url: String) -> HitbtcGatewayConfig {
    HitbtcGatewayConfig {
        rest_base_url,
        enabled: true,
        ..HitbtcGatewayConfig::default()
    }
}
