use rustcta_exchange_api::{
    ...
    ExchangeApiError, ExchangeClient, FeesRequest, OpenOrdersRequest, OrderListKind,
    OrderListLegType, OrderListOrderLeg, OrderListRequest, PlaceOrderRequest, QueryOrderRequest,
...
use super::private::{
    build_amend_order_replace_request_spec, build_batch_cancel_orders_request_spec,
    build_cancel_order_request_spec, build_get_balances_request_spec,
    build_get_fees_spot_request_spec, build_open_orders_request_spec,
    build_place_order_list_otoco_request_spec, build_place_order_request_spec,
    build_recent_fills_request_spec,
};
use super::private_parser::{
    parse_account_balances, parse_batch_cancel_orders_ack, parse_fee_snapshots,
    parse_order_list_ack, parse_spot_order_ack,
};
...
fn load_request_spec(name: &str) -> RequestSpec {
    let text = match name {
        ...
        "get_balances.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/request_specs/get_balances.json"
        ),
        "get_fees_spot.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/request_specs/get_fees_spot.json"
        ),
        ...
        "amend_order_replace.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/request_specs/amend_order_replace.json"
        ),
        "place_order_list_otoco.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/request_specs/place_order_list_otoco.json"
        ),
        "batch_cancel_orders.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/request_specs/batch_cancel_orders.json"
        ),
...
fn load_signing_vector(name: &str) -> SigningVector {
    let text = match name {
        ...
        "get_fees_spot_hs256.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/signing_vectors/get_fees_spot_hs256.json"
        ),
        ...
        "amend_order_replace_hs256.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/signing_vectors/amend_order_replace_hs256.json"
        ),
        "place_order_list_otoco_hs256.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/signing_vectors/place_order_list_otoco_hs256.json"
        ),
        "batch_cancel_orders_hs256.json" => include_str!(
            "../../../../../tests/fixtures/exchanges/hitbtc/signing_vectors/batch_cancel_orders_hs256.json"
        ),
...
fn load_parser_fixture(name: &str) -> serde_json::Value {
    let text = match name {
        ...
        "spot_balances.json" => {
            include_str!("../../../../../tests/fixtures/exchanges/hitbtc/parser/spot_balances.json")
        }
        "spot_fees.json" => {
            include_str!("../../../../../tests/fixtures/exchanges/hitbtc/parser/spot_fees.json")
        }
        "amend_order_ack.json" => {
            include_str!(
                "../../../../../tests/fixtures/exchanges/hitbtc/parser/amend_order_ack.json"
            )
        }
        "order_list_otoco_ack.json" => {
            include_str!(
                "../../../../../tests/fixtures/exchanges/hitbtc/parser/order_list_otoco_ack.json"
            )
        }
        "batch_cancel_orders_ack.json" => {
            include_str!(
                "../../../../../tests/fixtures/exchanges/hitbtc/parser/batch_cancel_orders_ack.json"
            )
        }
...
fn load_unsupported_boundary() -> serde_json::Value {
    serde_json::from_str(include_str!(
        "../../../../../tests/fixtures/exchanges/hitbtc/unsupported_boundary.json"
    ))
    .expect("unsupported boundary")
}

#[tokio::test]
async fn hitbtc_adapter_should_guard_private_operations_without_credentials() {
    ...
    for operation in ["hitbtc.amend_order", "hitbtc.place_order_list"] {
        ...
        assert_eq!(endpoint.auth, rustcta_exchange_api::EndpointAuth::Hmac);
    }
    let batch_place_endpoint = capabilities
        .capabilities_v2
        .endpoints
        .iter()
        .find(|endpoint| endpoint.operation == "hitbtc.batch_place_orders")
        .expect("batch place endpoint");
    assert!(matches!(
        batch_place_endpoint.support,
        CapabilitySupport::Unsupported { .. }
    ));
    assert_eq!(
        batch_place_endpoint.path.as_deref(),
        Some("/unsupported/hitbtc/batch_place_orders")
    );
    let batch_cancel_endpoint = capabilities
        .capabilities_v2
        .endpoints
        .iter()
        .find(|endpoint| endpoint.operation == "hitbtc.batch_cancel_orders")
        .expect("batch cancel endpoint");
    assert!(matches!(
        batch_cancel_endpoint.support,
        CapabilitySupport::Unsupported { .. }
    ));
    assert_eq!(batch_cancel_endpoint.method.as_deref(), Some("DELETE"));
    assert_eq!(batch_cancel_endpoint.path.as_deref(), Some("/spot/order"));
...
#[tokio::test]
async fn hitbtc_generic_batch_orders_should_stay_explicit_unsupported_boundaries() {
    let adapter = HitbtcGatewayAdapter::default_public().expect("adapter");
    ...
    assert!(matches!(
        ...
        ExchangeApiError::Unsupported {
            operation: "hitbtc.batch_place_orders_not_mapped"
        }
    ));
    ...
    let batch_cancel_error = adapter
        .batch_cancel_orders(BatchCancelOrdersRequest {
            ...
            cancels: vec![CancelOrderRequest {
                ...
                symbol: symbol_scope(),
                client_order_id: Some("cli-hitbtc-1".to_string()),
    ...
    assert!(matches!(
        batch_cancel_error,
        ExchangeApiError::Unsupported {
            operation: "hitbtc.batch_cancel_orders_filter_endpoint_overcancel_boundary"
        }
    ));
}

#[test]
fn hitbtc_unsupported_boundary_should_pin_generic_batch_audit_reasons() {
    let boundary = load_unsupported_boundary();
    let advanced = boundary
        .get("advanced_order_boundaries")
        .expect("advanced order boundaries");

    let batch_place = advanced
        .get("batch_place_orders")
        .expect("batch place boundary");
    assert_eq!(
        batch_place.get("status").and_then(|value| value.as_str()),
        Some("unsupported")
    );
    assert_eq!(
        batch_place
        ...
        Some("hitbtc.batch_place_orders_not_mapped")
    );
    assert!(batch_place
        .get("source_boundary")
        .and_then(|value| value.as_str())
        .expect("batch place source boundary")
        .contains("no lossless shared mapping"));

    let batch_cancel = advanced
        .get("batch_cancel_orders")
        .expect("batch cancel boundary");
    assert_eq!(
        batch_cancel
            .get("runtime_error")
            .and_then(|value| value.as_str()),
        Some("hitbtc.batch_cancel_orders_filter_endpoint_overcancel_boundary")
    );
    assert_eq!(
        batch_cancel
            .get("request_spec_fixture")
            .and_then(|value| value.as_str()),
        Some("tests/fixtures/exchanges/hitbtc/request_specs/batch_cancel_orders.json")
    );
    assert!(batch_cancel
        .get("source_boundary")
        .and_then(|value| value.as_str())
        .expect("batch cancel source boundary")
        .contains("over-cancel"));
}

#[tokio::test]
async fn hitbtc_fees_should_send_signed_private_rest_request() {
...
#[tokio::test]
async fn hitbtc_balances_should_send_signed_private_rest_request() {
    ...
    assert_eq!(response.balances[0].balances[0].available, 0.01);
    assert_eq!(response.balances[0].balances[0].locked, 0.002);
...
#[tokio::test]
async fn hitbtc_advanced_orders_should_send_signed_private_rest_requests() {
    let (base_url, seen) = spawn_rest_server(vec![
        load_parser_fixture("amend_order_ack.json"),
        load_parser_fixture("order_list_otoco_ack.json"),
    ])
    .await;
    let adapter = HitbtcGatewayAdapter::new(config_with_base_url(base_url)).expect("adapter");
    let capabilities = adapter.capabilities();
    ...
    let amended = adapter
        .amend_order(AmendOrderRequest {
            ...
            client_order_id: Some("cli-hitbtc-1".to_string()),
            exchange_order_id: None,
            new_client_order_id: Some("cli-hitbtc-1-r1".to_string()),
    ...
    let order_list = adapter
        .place_order_list(OrderListRequest::Oto {
            ...
            symbol: symbol_scope(),
            list_client_order_id: Some("cli-hitbtc-otoco".to_string()),
            working: OrderListOrderLeg {
                ...
                client_order_id: Some("cli-hitbtc-otoco-primary".to_string()),
            },
            pending: OrderListOrderLeg {
                ...
                stop_price: None,
                time_in_force: Some(TimeInForce::GTC),
                client_order_id: Some("cli-hitbtc-otoco-tp".to_string()),
            },
        })
        .await
        .expect("order list");
    ...
    assert_eq!(seen[0].path, "/api/3/spot/order/cli-hitbtc-1");
...
#[tokio::test]
async fn hitbtc_core_trading_should_send_signed_private_rest_requests() {
    ...
    assert!(seen[0].body.contains("client_order_id=cli-hitbtc-1"));
    assert!(seen[0].headers.contains_key("authorization"));
    assert_eq!(seen[1].method, "DELETE");
    assert_eq!(seen[1].path, "/api/3/spot/order/cli-hitbtc-1");
    assert_eq!(seen[2].method, "GET");
    assert_eq!(seen[2].path, "/api/3/spot/order/cli-hitbtc-1");
...
fn hitbtc_request_specs_should_match_offline_private_builders() {
    ...
    let amend = build_amend_order_replace_request_spec(
        "test-key",
        "test-secret",
        1_710_000_000_000,
        10_000,
    ...
    let order_list = build_place_order_list_otoco_request_spec(
        "test-key",
        "test-secret",
        1_710_000_000_000,
        10_000,
...
fn hitbtc_private_parser_should_parse_order_ack_fixture() {
    ...
    let amended = parse_spot_order_ack(
        &exchange_id(),
        symbol_scope(),
        &load_parser_fixture("amend_order_ack.json"),
    )
    .expect("amend ack");
    assert_eq!(amended.client_order_id.as_deref(), Some("cli-hitbtc-1-r1"));
    assert_eq!(amended.quantity, "0.050");

    let order_list = parse_order_list_ack(
        &exchange_id(),
        symbol_scope(),
        OrderListKind::Oto,
        &load_parser_fixture("order_list_otoco_ack.json"),
    ...
    assert_eq!(
        order_list.list_client_order_id.as_deref(),
        Some("cli-hitbtc-otoco")
    );
    assert_eq!(order_list.orders.len(), 3);

    let batch_cancel = parse_batch_cancel_orders_ack(
        &exchange_id(),
        symbol_scope(),
        &load_parser_fixture("batch_cancel_orders_ack.json"),
    )
    .expect("batch cancel ack");
    assert_eq!(batch_cancel.len(), 2);
    assert!(batch_cancel
        .iter()
