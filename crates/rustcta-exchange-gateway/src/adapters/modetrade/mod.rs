...
pub use config::ModetradeGatewayConfig;
use transport::ModetradeRest;

#[derive(Clone)]
pub struct ModetradeGatewayAdapter {
    exchange_id: ExchangeId,
    config: ModetradeGatewayConfig,
    rest: ModetradeRest,
}

impl ModetradeGatewayAdapter {
    pub fn new(config: ModetradeGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("modetrade").map_err(validation_error)?;
        let rest = ModetradeRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("modetrade adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "modetrade.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for ModetradeGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "modetrade Orderly perpetual profile: public info parser plus Unsupported private trading boundary"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for ModetradeGatewayAdapter {
