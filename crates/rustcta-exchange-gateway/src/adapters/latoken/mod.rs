...
pub use config::LatokenGatewayConfig;
use transport::LatokenRest;

#[derive(Clone)]
pub struct LatokenGatewayAdapter {
    exchange_id: ExchangeId,
    config: LatokenGatewayConfig,
    rest: LatokenRest,
}

impl LatokenGatewayAdapter {
    pub fn new(config: LatokenGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("latoken").map_err(validation_error)?;
        let rest = LatokenRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(LatokenGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("LATOKEN adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "latoken.non_spot_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for LatokenGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "LATOKEN spot scan-only public REST gateway; private REST request-spec-only"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for LatokenGatewayAdapter {
    ...
    async fn get_positions(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("latoken.positions_unsupported_spot_only")
    }
    ...
    async fn place_quote_market_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_spot(request.symbol.market_type)?;
        self.unsupported("latoken.quote_market_order_unverified")
    }
    ...
    async fn amend_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_spot(request.symbol.market_type)?;
        self.unsupported("latoken.amend_order_unsupported")
    }
    ...
    async fn place_order_list(
        ...
        self.ensure_exchange(&request.symbol().exchange)?;
        self.ensure_spot(request.symbol().market_type)?;
        self.unsupported("latoken.order_list_unsupported")
    }
