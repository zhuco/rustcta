...
fn parse_symbol_rule(
    ...
    let (fallback_base, fallback_quote) = split_bingx_symbol(&exchange_symbol)
    ...
    Ok(SymbolRules {
        ...
        max_quantity: string_or_number(value.get("maxQty").or_else(|| value.get("maxTradeQty"))),
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "bingx order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_bingx_symbol(symbol: &str, market_type: MarketType) -> ExchangeApiResult<String> {
    ...
    let (base, quote) =
        split_compact_symbol(&compact).ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: format!("cannot normalize BingX symbol {symbol}"),
        })?;
    let _ = market_type;
...
pub(super) fn split_bingx_symbol(symbol: &str) -> Option<(String, String)> {
