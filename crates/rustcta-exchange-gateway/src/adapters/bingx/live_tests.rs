...
#[tokio::test]
#[ignore]
async fn bingx_live_readonly_should_cover_public_and_authenticated_baseline() {
    let Some(api_key) = non_empty_env("BINGX_API_KEY") else {
        eprintln!("BINGX_API_KEY missing; skipping BingX authenticated live readonly test");
        return;
    };
    let Some(api_secret) = non_empty_env("BINGX_API_SECRET") else {
        eprintln!("BINGX_API_SECRET missing; skipping BingX authenticated live readonly test");
        return;
    };
    ...
    let rules = adapter
        .get_symbol_rules(SymbolRulesRequest {
            ...
            context: context("bingx-live-rules"),
    ...
    let spot_book = adapter
        .get_order_book(OrderBookRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("bingx-live-spot-book"),
    ...
    let perp_book = adapter
        .get_order_book(OrderBookRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("bingx-live-perp-book"),
    ...
    adapter
        .get_fees(FeesRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("bingx-live-fees"),
    ...
    adapter
        .get_balances(BalancesRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("bingx-live-balances"),
    ...
    adapter
        .get_open_orders(OpenOrdersRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("bingx-live-open-orders"),
            exchange: exchange_id(),
            market_type: Some(MarketType::Spot),
