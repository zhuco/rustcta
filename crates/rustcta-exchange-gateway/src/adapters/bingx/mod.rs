...
pub use config::BingxGatewayConfig;
use transport::BingxRest;

#[derive(Clone)]
pub struct BingxGatewayAdapter {
    exchange_id: ExchangeId,
    config: BingxGatewayConfig,
    rest: BingxRest,
}

impl BingxGatewayAdapter {
    pub fn new(config: BingxGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("bingx").map_err(validation_error)?;
        let rest = BingxRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BingxGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("bingx adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "bingx.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for BingxGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "bingx spot + linear perpetual public/private REST gateway adapter"
                } else {
                    "bingx spot + linear perpetual public REST gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for BingxGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities =
            Some(streams::bingx_private_stream_capabilities(private));
        ...
        capabilities.supports_order_list = private;
