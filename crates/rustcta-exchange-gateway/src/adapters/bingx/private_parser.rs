...
use super::parser::{
    ...
    parse_side, required_str, split_bingx_symbol, string_or_number, validation_error,
    value_as_string,
};
...
pub fn parse_positions(
    ...
    for position in positions {
        ...
        let (base, quote) = split_bingx_symbol(&symbol)
    ...
    for item in items {
        ...
        fees.push(FeeRateSnapshot {
            ...
            source: Some(if market_type == MarketType::Spot {
                "bingx.spot.commission_rate".to_string()
            } else {
                "bingx.swap.commission_rate".to_string()
            }),
            updated_at: Utc::now(),
        });
    }
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "bingx fill requires canonical_symbol".to_string(),
            })?;
...
fn symbol_from_payload(
    ...
    let (base, quote) = split_bingx_symbol(&exchange_symbol)
        .unwrap_or_else(|| ("UNKNOWN".to_string(), "USDT".to_string()));
    Ok(SymbolScope {
        exchange: exchange_id.clone(),
        market_type,
...
fn parse_order_type(raw_type: &str, tif: Option<TimeInForce>) -> OrderType {
    match raw_type.to_ascii_uppercase().as_str() {
        ...
        "STOP" | "TAKE_PROFIT" | "TRIGGER_LIMIT" | "OCOTPS" => OrderType::StopLimit,
        _ => match tif {
...
fn map_bingx_order_status(status: &str) -> OrderStatus {
