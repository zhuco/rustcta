...
pub fn parse_aster_cancel_all_orders(
    exchange_id: &ExchangeId,
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "aster recent fills request requires canonical_symbol".to_string(),
            })?;
...
fn split_linear_symbol(symbol: &str) -> (&str, &str) {
    let pair = symbol
        ...
        .map(|(pair, _)| pair)
        .unwrap_or(symbol);
    if let Some(base) = pair.strip_suffix("USDT") {
        (base, "USDT")
    } else if let Some(base) = pair.strip_suffix("USD") {
        (base, "USD")
    } else {
        (pair, "USD")
    }
...
fn parse_fee_snapshot(
    ...
    Ok(FeeRateSnapshot {
        ...
        maker_rate: maker,
        taker_rate: taker,
        source: Some("aster.commission_rate".to_string()),
        updated_at: Utc::now(),
    })
}

fn map_aster_order_status(status: &str) -> OrderStatus {
