...
fn load_ws_fixture(name: &str) -> serde_json::Value {
    let text = match name {
        "orderbook_partial.json" => {
            include_str!("../../../../../tests/fixtures/exchanges/fmfwio/ws/orderbook_partial.json")
        }
        _ => panic!("unknown fmfwio ws fixture {name}"),
    };
    serde_json::from_str(text).expect("ws fixture")
}

#[test]
fn fmfwio_ws_payloads_should_cover_subscribe_unsubscribe_and_auth() {
...
#[test]
fn fmfwio_ws_orderbook_fixture_should_parse_snapshot_for_rest_resync() {
...
#[test]
fn fmfwio_ws_heartbeat_policy_should_track_server_ping() {
