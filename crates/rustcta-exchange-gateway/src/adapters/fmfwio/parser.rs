...
pub fn parse_symbol_rules(
    ...
    let symbols = value.as_object().ok_or_else(|| {
        parse_error(
            exchange_id.clone(),
            "FMFW.io symbol response missing object",
            value,
        )
    })?;
    symbols
    ...
    Ok(SymbolRules {
        ...
        symbol: SymbolScope {
            ...
            exchange_symbol: ExchangeSymbol::new(
                exchange_id.clone(),
                MarketType::Spot,
                normalize_fmfwio_symbol_text(symbol)?,
            )
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "fmfwio order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_fmfwio_symbol(scope: &SymbolScope) -> ExchangeApiResult<String> {
    ...
    normalize_fmfwio_symbol_text(&scope.exchange_symbol.symbol)
}
...
pub(super) fn symbol_scope_from_exchange_symbol(
    ...
    let exchange_symbol = normalize_fmfwio_symbol_text(exchange_symbol)?;
    let (base, quote) =
        split_symbol_guess(&exchange_symbol).ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: format!("cannot infer canonical FMFW.io symbol from {exchange_symbol}"),
        })?;
    Ok(SymbolScope {
        exchange: exchange_id.clone(),
        market_type: MarketType::Spot,
...
pub(super) fn normalize_fmfwio_symbol_text(symbol: &str) -> ExchangeApiResult<String> {
    ...
    if normalized.is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "FMFW.io symbol must not be empty".to_string(),
        });
    }
    if normalized.ends_with("PERP") {
        return Err(ExchangeApiError::Unsupported {
            operation: "fmfwio.derivatives_profile_not_enabled",
        });
    }
    Ok(normalized)
}
...
fn parse_levels(
    ...
    let levels = levels.and_then(Value::as_array).ok_or_else(|| {
        parse_error(
            exchange_id.clone(),
            "FMFW.io order book missing price levels",
            &Value::Null,
        )
    })?;
    levels
        ...
        .map(|level| {
            let array = level.as_array().ok_or_else(|| {
                parse_error(
                    exchange_id.clone(),
                    "FMFW.io order book level is not an array",
                    level,
                )
            })?;
            let price = array
