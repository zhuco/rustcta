...
use super::private::FmfwioOfflineRequest;

#[derive(Clone)]
pub struct FmfwioRest {
    ...
    pub async fn send_signed_request(
        ...
        let method = reqwest::Method::from_bytes(request.method.as_bytes()).map_err(|error| {
            ExchangeApiError::InvalidRequest {
                message: format!("invalid FMFW.io private REST method: {error}"),
            }
        })?;
...
async fn parse_response(
    ...
    let message = value
        ...
        .unwrap_or("FMFW.io request failed");
    let mut error = ExchangeError::new(
        exchange_id,
        classify_fmfwio_error(message, status.as_u16()),
        message,
        Utc::now(),
    );
...
fn classify_fmfwio_error(message: &str, status: u16) -> ExchangeErrorClass {
