...
use super::parser::{normalize_coinmetro_symbol, parse_orderbook_snapshot, parse_symbol_rules};
...
impl CoinmetroGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let requested = request
            .symbols
            .iter()
            .map(|symbol| normalize_coinmetro_symbol(&symbol.exchange_symbol.symbol))
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let market = normalize_coinmetro_symbol(&request.symbol.exchange_symbol.symbol)?;
        let value = self
            .rest
            .send_public_request(&format!("/exchange/book/{market}"), &HashMap::new())
            .await?;
