    ...
    tokio::spawn(async move {
        loop {
            ...
            let body = responses
                ...
                .unwrap()
                .next()
                .unwrap_or_else(|| json!({ "status": 1, "data": {} }));
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("bitbns").expect("bitbns exchange")
}
...
pub(super) fn symbol_scope(symbol: &str) -> SymbolScope {
    let (base, quote) = symbol.split_once('_').expect("BASE_QUOTE symbol");
    SymbolScope {
