...
pub use config::BtcMarketsGatewayConfig;
use transport::BtcMarketsRest;

#[derive(Clone)]
pub struct BtcMarketsGatewayAdapter {
    exchange_id: ExchangeId,
    config: BtcMarketsGatewayConfig,
    rest: BtcMarketsRest,
}

impl BtcMarketsGatewayAdapter {
    pub fn new(config: BtcMarketsGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("btcmarkets").map_err(validation_error)?;
        let rest = BtcMarketsRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BtcMarketsGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("btcmarkets adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "btcmarkets private request requires context.tenant_id".to_string(),
                })?;
        let account_id =
            context
                .account_id
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "btcmarkets private request requires context.account_id".to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for BtcMarketsGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("btcmarkets Australian AUD spot REST/WS gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for BtcMarketsGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities =
            Some(streams::btcmarkets_private_stream_capabilities(private));
