...
use super::parser::{normalize_exmo_exchange_symbol, parse_orderbook_snapshot, parse_symbol_rules};
...
impl ExmoGatewayAdapter {
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let limit = request.depth.unwrap_or(100).clamp(1, 1000).to_string();
        let params = vec![("pair".to_string(), pair), ("limit".to_string(), limit)];
        let value = self
            .rest
            .send_public_post("/v1.1/order_book", &params)
            .await?;
