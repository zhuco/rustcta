...
pub use config::MangoMarketsGatewayConfig;

#[derive(Clone)]
pub struct MangoMarketsGatewayAdapter {
    exchange_id: ExchangeId,
    config: MangoMarketsGatewayConfig,
}

impl MangoMarketsGatewayAdapter {
    pub fn new(config: MangoMarketsGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("mango_markets").map_err(validation_error)?;
        Ok(Self {
            exchange_id,
            config,
        })
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "mango_markets adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "mango_markets.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for MangoMarketsGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "mango_markets Solana margin/perp G0 adapter: scan-only audit with explicit Unsupported trading boundary"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for MangoMarketsGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.supports_private_rest = self.config.private_runtime_available();
