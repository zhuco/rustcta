...
pub fn parse_order_state(
    ...
    Ok(OrderState {
        ...
        order_type: parse_order_type(raw_type, tif_text),
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "mexc recent fills request requires canonical_symbol".to_string(),
            })?;
...
fn parse_fee_snapshot(
    ...
    let maker = string_or_number(value.get("makerCommission").or_else(|| value.get("maker")))
        .ok_or_else(|| {
            parse_error(
                exchange_id.clone(),
    ...
    let taker = string_or_number(value.get("takerCommission").or_else(|| value.get("taker")))
        .ok_or_else(|| {
            parse_error(
                exchange_id.clone(),
    ...
    Ok(FeeRateSnapshot {
        ...
        maker_rate: maker,
        taker_rate: taker,
        source: Some("mexc.trade_fee".to_string()),
        updated_at: Utc::now(),
    })
}

fn map_mexc_order_status(status: &str) -> OrderStatus {
