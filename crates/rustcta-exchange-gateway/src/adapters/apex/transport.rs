...
pub fn signed_get_request_spec(path: &str) -> Value {
    json!({
        "method": "GET",
        "path": path,
        "auth": "apex_api_key_hmac",
        "headers": {
            "APEX-SIGNATURE": "<base64-hmac>",
            "APEX-TIMESTAMP": "<iso-or-ms-timestamp>",
            "APEX-API-KEY": "<redacted>",
            "APEX-PASSPHRASE": "<redacted>"
        }
    })
}
...
async fn parse_response(
    ...
    let code_failed = value
        .get("code")
        .and_then(Value::as_str)
        .is_some_and(|code| code != "0");
    if !status.is_success() || code_failed {
        let message = value
            .get("message")
            .or_else(|| value.get("msg"))
            .and_then(Value::as_str)
            .unwrap_or("ApeX request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_apex_error(status.as_u16(), message),
            message,
            Utc::now(),
        );
        error.code = value
...
fn classify_apex_error(status: u16, message: &str) -> ExchangeErrorClass {
