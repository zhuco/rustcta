...
pub const WOOFIPRO_ORDERLY_SIGNING_BOUNDARY: &str =
    "woofipro uses Orderly Ed25519 account headers; private REST requires explicit account id, orderly key and base58/hex orderly secret";
...
pub fn woofipro_orderly_canonical_payload(
    timestamp_ms: i64,
    method: &str,
    path_with_query: &str,
    body: &str,
...
pub fn woofipro_orderly_signing_boundary() -> &'static str {
    WOOFIPRO_ORDERLY_SIGNING_BOUNDARY
}
...
pub fn sign_orderly_request(
    ...
    let canonical_payload =
        woofipro_orderly_canonical_payload(timestamp_ms, method, path_with_query, body);
...
fn decode_orderly_secret(secret: &str) -> ExchangeApiResult<[u8; 32]> {
    ...
    bytes
        ...
        .map_err(|bytes: Vec<u8>| ExchangeApiError::InvalidRequest {
            message: format!(
                "woofipro orderly secret must decode to 32 bytes, got {} bytes",
                bytes.len()
            ),
        })
}

fn decode_hex_32(value: &str) -> ExchangeApiResult<Vec<u8>> {
    hex::decode(value).map_err(|error| ExchangeApiError::InvalidRequest {
...
fn decode_base58(value: &str) -> ExchangeApiResult<Vec<u8>> {
    ...
    if value.trim().is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "woofipro orderly secret must not be empty".to_string(),
        });
    }
    ...
    for ch in value.bytes() {
        let carry = ALPHABET
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "invalid base58 character in woofipro orderly secret".to_string(),
            })? as u32;
        let mut carry = carry;
