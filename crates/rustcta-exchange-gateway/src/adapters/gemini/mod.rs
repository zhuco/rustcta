...
pub use config::GeminiGatewayConfig;
use transport::GeminiRest;

#[derive(Clone)]
pub struct GeminiGatewayAdapter {
...
impl GeminiGatewayAdapter {
    pub fn new(config: GeminiGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("gemini").map_err(validation_error)?;
        let rest = GeminiRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("gemini adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "gemini.non_spot_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "gemini private REST request requires tenant_id".to_string(),
                })?;
        let account_id =
            context
                .account_id
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "gemini private REST request requires account_id".to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for GeminiGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("gemini spot REST/WS gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for GeminiGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities =
            Some(streams::gemini_private_stream_capabilities(private));
    ...
    async fn get_positions(
        ...
        self.unsupported("gemini.positions_unsupported_spot_only")
    }
    ...
    async fn get_fees(&self, request: FeesRequest) -> ExchangeApiResult<FeesResponse> {
        ...
        self.unsupported("gemini.fees_endpoint_not_mapped")
    }
    ...
    async fn place_quote_market_order(
        ...
        self.unsupported("gemini.quote_market_order_unsupported")
    }
    ...
    async fn batch_place_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("gemini.batch_place_orders_composed_not_exposed")
    }
    ...
    async fn batch_cancel_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.unsupported("gemini.batch_cancel_orders_composed_not_exposed")
    }
    ...
    async fn subscribe_public_stream(
        ...
        Ok(streams::public_marketdata_url(
    ...
    async fn subscribe_private_stream(
        ...
        self.ensure_private_rest("gemini.private_stream_auth")?;
        Ok(streams::private_subscription_marker().to_string())
    }
}
