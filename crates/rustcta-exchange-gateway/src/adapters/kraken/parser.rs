...
pub fn parse_spot_symbol_rules(
    ...
    reject_kraken_error_response(exchange_id, value)?;
    let object = value.as_object().ok_or_else(|| {
        parse_error(
            exchange_id.clone(),
            "Kraken AssetPairs response missing object",
            value,
        )
    })?;
    object
        .iter()
        .filter_map(|(pair_code, pair)| {
            parse_spot_symbol_rule(exchange_id, pair_code, pair).transpose()
        })
        .collect()
}
...
fn parse_spot_symbol_rule(
    ...
    let Some(canonical) = value
        .get("wsname")
        .and_then(Value::as_str)
        .and_then(|name| canonical_from_pair(name).ok())
        .or_else(|| canonical_from_pair(altname).ok())
    else {
        return Ok(None);
    };
    let market_type = MarketType::Spot;
    let exchange_symbol =
        ExchangeSymbol::new(exchange_id.clone(), market_type, altname).map_err(validation_error)?;
    ...
    let pair_decimals = value.get("pair_decimals").and_then(u32_from_value);
    let lot_decimals = value.get("lot_decimals").and_then(u32_from_value);
    Ok(Some(SymbolRules {
        ...
        price_increment: string_or_number(value.get("tick_size"))
            .or_else(|| pair_decimals.map(decimal_step)),
        quantity_increment: lot_decimals.map(decimal_step),
        min_price: None,
        max_price: None,
        min_quantity: string_or_number(value.get("ordermin")),
        max_quantity: None,
        min_notional: string_or_number(value.get("costmin")),
        max_notional: None,
...
pub fn parse_spot_orderbook_snapshot(
    ...
    let book = value
        ...
        .ok_or_else(|| parse_error(exchange_id.clone(), "Kraken depth missing book", value))?;
    parse_orderbook(
        exchange_id,
        symbol,
        book,
...
pub fn normalize_spot_symbol(symbol: &SymbolScope) -> ExchangeApiResult<String> {
    if symbol.market_type != MarketType::Spot {
        return Err(ExchangeApiError::Unsupported {
            operation: "kraken.non_spot_symbol",
        });
    }
    Ok(symbol.exchange_symbol.symbol.trim().to_ascii_uppercase())
}

pub fn normalize_futures_symbol(symbol: &SymbolScope) -> ExchangeApiResult<String> {
    if symbol.market_type != MarketType::Perpetual {
        return Err(ExchangeApiError::Unsupported {
            operation: "kraken.non_perpetual_symbol",
        });
    }
    ...
    let base = kraken_asset(canonical.base_asset());
...
pub fn kraken_asset(asset: &str) -> String {
...
pub(super) fn u32_from_value(value: &Value) -> Option<u32> {
    match value {
        Value::String(text) => text.parse().ok(),
...
fn decimal_step(precision: u32) -> String {
    if precision == 0 {
        "1".to_string()
    } else {
...
pub(super) fn reject_kraken_error_response(
    exchange_id: &ExchangeId,
    value: &Value,
) -> ExchangeApiResult<()> {
...
mod tests {
    ...
    fn exchange_id() -> ExchangeId {
        ExchangeId::new("kraken").expect("exchange")
    }

    #[test]
    fn kraken_spot_symbol_rules_should_parse_asset_pairs() {
        let value = json!({
            "XXBTZUSD": {
                "altname": "XBTUSD",
                "wsname": "XBT/USD",
        ...
        let rules = parse_spot_symbol_rules(&exchange_id(), &value).expect("rules");
        ...
        assert_eq!(rules[0].min_quantity.as_deref(), Some("0.00005"));
    }

    #[test]
    fn kraken_futures_symbol_rules_should_parse_linear_perp() {
        let value = json!({
            "instruments": [{
                "symbol": "PF_XBTUSDT",
                "type": "perpetual",
