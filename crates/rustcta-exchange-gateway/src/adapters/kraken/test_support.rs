...
fn parse_seen_request(request_text: &str) -> SeenRequest {
    ...
    SeenRequest {
        method,
        path: path.to_string(),
        query: parse_pairs(query_text),
        form: parse_pairs(body),
        headers,
    }
}
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("kraken").expect("kraken exchange")
}
...
pub(super) fn spot_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "XBTUSD")
            .expect("symbol"),
    }
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "PF_XBTUSDT")
            .expect("symbol"),
    }
}

pub(super) fn private_config(base_url: String) -> super::KrakenGatewayConfig {
    super::KrakenGatewayConfig {
        spot_rest_base_url: base_url.clone(),
        futures_rest_base_url: base_url,
        spot_api_key: Some("key".to_string()),
        spot_api_secret: Some(
            "kQH5HW/8p1uGOVjbgWA7FunAmGO8lsSUXNsu3eow76sz84Q18fWxnyRzBHCd3pd5nE9qa99HAZtuZuj6F1huXg=="
                .to_string(),
        ),
        futures_api_key: Some("futures-key".to_string()),
        futures_api_secret: Some(
            "kQH5HW/8p1uGOVjbgWA7FunAmGO8lsSUXNsu3eow76sz84Q18fWxnyRzBHCd3pd5nE9qa99HAZtuZuj6F1huXg=="
                .to_string(),
        ),
        enabled_private_rest: true,
        ..super::KrakenGatewayConfig::default()
    }
}
