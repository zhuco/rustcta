...
pub use config::HashKeyGlobalGatewayConfig;
use transport::HashKeyGlobalRest;

#[derive(Clone)]
pub struct HashKeyGlobalGatewayAdapter {
    exchange_id: ExchangeId,
    config: HashKeyGlobalGatewayConfig,
    rest: HashKeyGlobalRest,
}

impl HashKeyGlobalGatewayAdapter {
    pub fn new(config: HashKeyGlobalGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("hashkey_global").map_err(validation_error)?;
        let rest = HashKeyGlobalRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(HashKeyGlobalGatewayConfig::default())
    }
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "hashkey_global adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "hashkey_global.unsupported_market_type",
            });
        }
        Ok(())
    }
    ...
    async fn send_signed_get(
        ...
        if is_futures_endpoint(endpoint) {
            self.rest
    ...
    async fn send_signed_delete(
        ...
        if is_futures_endpoint(endpoint) {
            self.rest
                .send_futures_signed_delete(endpoint, params, api_key, api_secret, recv_window_ms)
                .await
        } else {
            self.rest
...
fn is_futures_endpoint(endpoint: &str) -> bool {
    endpoint.starts_with("/api/v1/futures/")
}

#[async_trait]
impl GatewayAdapter for HashKeyGlobalGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "hashkey_global spot + linear perpetual public/private REST gateway adapter"
                } else {
                    "hashkey_global spot + linear perpetual public REST gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for HashKeyGlobalGatewayAdapter {
    ...
    async fn place_quote_market_order(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "hashkey_global.place_quote_market_order",
        })
    }
