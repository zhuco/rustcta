...
fn parse_symbol_rule(
    ...
    let (fallback_base, fallback_quote) = split_hashkey_global_symbol(&exchange_symbol)
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "hashkey_global order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_hashkey_global_symbol(
    symbol: &str,
    market_type: MarketType,
    ...
    if market_type == MarketType::Perpetual {
        ...
        let (base, quote) =
            split_compact_symbol(&compact).ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: format!("cannot normalize HashKeyGlobal futures symbol {symbol}"),
            })?;
    ...
    let (base, quote) =
        split_compact_symbol(&compact).ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: format!("cannot normalize HashKeyGlobal symbol {symbol}"),
        })?;
...
pub(super) fn split_hashkey_global_symbol(symbol: &str) -> Option<(String, String)> {
