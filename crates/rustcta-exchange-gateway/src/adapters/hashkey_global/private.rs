...
use super::parser::normalize_hashkey_global_symbol;
...
impl HashKeyGlobalGatewayAdapter {
    pub(super) async fn get_balances_impl(
        ...
        let (tenant_id, account_id) =
            self.context_account(&request.context, "hashkey_global.get_balances")?;
        let endpoint = match market_type {
            MarketType::Spot => "/api/v1/account",
            MarketType::Perpetual => "/api/v1/futures/account",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        let value = self
            .send_signed_get("hashkey_global.get_balances", endpoint, &HashMap::new())
            .await?;
    ...
    pub(super) async fn get_positions_impl(
            ...
            .is_some_and(|market| market != MarketType::Perpetual)
        {
            return Err(ExchangeApiError::Unsupported {
                operation: "hashkey_global.positions_non_perpetual",
            });
        }
        let (tenant_id, account_id) =
            self.context_account(&request.context, "hashkey_global.get_positions")?;
        let mut params = HashMap::new();
        if let Some(symbol) = request.symbols.first() {
            if symbol.exchange_id != self.exchange_id {
                return Err(ExchangeApiError::InvalidRequest {
                    message: format!(
                        "hashkey_global adapter cannot serve position request for exchange {}",
                        symbol.exchange_id
                    ),
                });
            }
            params.insert(
                "symbol".to_string(),
                normalize_hashkey_global_symbol(&symbol.symbol, MarketType::Perpetual)?,
            );
        }
        let value = self
            .send_signed_get(
                "hashkey_global.get_positions",
    ...
    pub(super) async fn get_fees_impl(
        ...
        if request.symbols.is_empty() {
            return Err(ExchangeApiError::InvalidRequest {
                message: "hashkey_global get_fees requires at least one symbol".to_string(),
            });
        }
        ...
        for symbol in &request.symbols {
            ...
            let endpoint = match symbol.market_type {
                MarketType::Spot => "/api/v1/account",
                MarketType::Perpetual => "/api/v1/futures/commissionRate",
                _ => unreachable!("checked by ensure_supported_market"),
            };
            let mut params = HashMap::new();
            if symbol.market_type == MarketType::Perpetual {
                params.insert(
                    "symbol".to_string(),
                    normalize_hashkey_global_symbol(
            ...
            let value = self
                .send_signed_get("hashkey_global.get_fees", endpoint, &params)
                .await?;
    ...
    pub(super) async fn place_order_impl(
        ...
        let params = hashkey_global_place_order_params(&request)?;
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => "/api/v1/order",
            MarketType::Perpetual => "/api/v1/futures/order",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        let value = self
            .send_signed_post("hashkey_global.place_order", endpoint, &params)
            .await?;
    ...
    pub(super) async fn cancel_order_impl(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_supported_market(request.symbol.market_type)?;
        let params = hashkey_global_cancel_order_params(&request)?;
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => "/api/v1/order",
            MarketType::Perpetual => "/api/v1/futures/order",
            _ => unreachable!("checked by ensure_supported_market"),
        ...
        let value = match request.symbol.market_type {
            MarketType::Spot => {
                self.send_signed_delete("hashkey_global.cancel_order", endpoint, &params)
                    .await?
            }
            MarketType::Perpetual => {
                self.send_signed_post("hashkey_global.cancel_order", endpoint, &params)
                    .await?
            }
            _ => unreachable!("checked by ensure_supported_market"),
        };
    ...
    pub(super) async fn batch_place_orders_impl(
        ...
        if request.orders.is_empty() {
            return Err(ExchangeApiError::InvalidRequest {
                message: "hashkey_global batch_place_orders requires at least one order"
                    .to_string(),
            });
        }
    ...
    pub(super) async fn batch_cancel_orders_impl(
        ...
        if request.cancels.is_empty() {
            return Err(ExchangeApiError::InvalidRequest {
                message: "hashkey_global batch_cancel_orders requires at least one cancel"
                    .to_string(),
            });
        }
    ...
    pub(super) async fn cancel_all_orders_impl(
        ...
        let symbol = request
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "hashkey_global cancel_all_orders requires symbol".to_string(),
            })?;
        self.ensure_exchange(&symbol.exchange)?;
        self.ensure_supported_market(symbol.market_type)?;
        if request
        ...
        {
            return Err(ExchangeApiError::InvalidRequest {
                message: "hashkey_global cancel_all_orders market_type does not match symbol"
                    .to_string(),
            });
        }
    ...
    pub(super) async fn query_order_impl(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_supported_market(request.symbol.market_type)?;
        let params = hashkey_global_query_order_params(&request)?;
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => "/api/v1/order",
            MarketType::Perpetual => "/api/v1/futures/order",
            _ => unreachable!("checked by ensure_supported_market"),
        ...
        let value = self
            .send_signed_get("hashkey_global.query_order", endpoint, &params)
            .await?;
    ...
    pub(super) async fn get_open_orders_impl(
        ...
        let endpoint = match market_type {
            ...
            MarketType::Perpetual => "/api/v1/futures/openOrders",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        let value = self
            .send_signed_get("hashkey_global.get_open_orders", endpoint, &params)
            .await?;
    ...
    pub(super) async fn get_recent_fills_impl(
        ...
        let symbol = request
            ...
            .as_ref()
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "hashkey_global get_recent_fills requires symbol".to_string(),
            })?;
        self.ensure_exchange(&symbol.exchange)?;
        self.ensure_supported_market(symbol.market_type)?;
        let (tenant_id, account_id) =
            self.context_account(&request.context, "hashkey_global.get_recent_fills")?;
        let mut params = HashMap::new();
        params.insert(
            (if symbol.market_type == MarketType::Perpetual {
                "symbol"
            ...
            .to_string(),
            normalize_hashkey_global_symbol(&symbol.exchange_symbol.symbol, symbol.market_type)?,
        );
        ...
        let endpoint = match symbol.market_type {
            ...
            MarketType::Perpetual => "/api/v1/futures/myTrades",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        let value = self
            .send_signed_get("hashkey_global.get_recent_fills", endpoint, &params)
            .await?;
...
fn hashkey_global_place_order_params(
    request: &PlaceOrderRequest,
) -> ExchangeApiResult<HashMap<String, String>> {
    if request.symbol.market_type == MarketType::Spot && request.reduce_only {
        return Err(ExchangeApiError::InvalidRequest {
            message: "hashkey_global spot order does not support reduce_only".to_string(),
        });
    }
    if request.quote_quantity.is_some() {
        return Err(ExchangeApiError::Unsupported {
            operation: "hashkey_global.quote_sized_order",
        });
    }
    if matches!(
        request.order_type,
        ...
        return Err(ExchangeApiError::Unsupported {
            operation: "hashkey_global.stop_order",
        });
    }
    if request.symbol.market_type == MarketType::Spot {
        ...
        {
            return Err(ExchangeApiError::Unsupported {
                operation: "hashkey_global.spot_advanced_time_in_force",
            });
        }
    }
    let mut params = HashMap::new();
    ...
    if request.order_type.requires_limit_price() {
        params.insert(
            ...
            request
                ...
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "hashkey_global limit-like order requires price".to_string(),
                })?,
        );
    ...
    if request.symbol.market_type == MarketType::Perpetual {
        ...
        if let Some(position_side) = request.position_side {
            params.insert(
                ...
                position_side_text(position_side).to_string(),
            );
        }
    }
    Ok(params)
...
fn hashkey_global_cancel_order_params(
    request: &CancelOrderRequest,
) -> ExchangeApiResult<HashMap<String, String>> {
    let mut params = HashMap::new();
    params.insert(
    ...
    {
        return Err(ExchangeApiError::InvalidRequest {
            message: "hashkey_global cancel_order requires exchange_order_id or client_order_id"
                .to_string(),
        });
    }
    Ok(params)
...
fn hashkey_global_query_order_params(
    request: &QueryOrderRequest,
) -> ExchangeApiResult<HashMap<String, String>> {
    let mut params = HashMap::new();
    params.insert(
    ...
    {
        return Err(ExchangeApiError::InvalidRequest {
            message: "hashkey_global query_order requires exchange_order_id or client_order_id"
                .to_string(),
        });
    }
    Ok(params)
