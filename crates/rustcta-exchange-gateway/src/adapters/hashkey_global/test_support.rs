...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("hashkey_global").expect("hashkey_global exchange")
}
...
pub(super) fn assert_signed_hashkey_global_request(
    request: &SeenRequest,
    method: &str,
    path: &str,
) {
    ...
    assert_eq!(
        request.headers.get("x-hk-apikey").map(String::as_str),
        Some("key")
    );
    assert!(request
        .query
