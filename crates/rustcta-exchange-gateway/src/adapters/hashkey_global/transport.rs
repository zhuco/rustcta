...
impl HashKeyGlobalRest {
    ...
    pub async fn send_futures_signed_post(
        ...
        let params = json_body_to_params(body)?;
    ...
    pub async fn send_futures_signed_delete(
        &self,
        endpoint: &str,
        params: &HashMap<String, String>,
        api_key: &str,
...
fn json_body_to_params(body: &Value) -> ExchangeApiResult<HashMap<String, String>> {
    let object = body
        .as_object()
        .ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: "HashKeyGlobal signed body must be a JSON object".to_string(),
        })?;
    Ok(object
        .iter()
        .filter_map(|(key, value)| {
            ...
            Some((key.clone(), text.trim_matches('"').to_string()))
        })
        .collect())
}
...
async fn parse_response(
    ...
    if !status.is_success() || code.as_deref().is_some_and(|code| code != "0") {
        let message = value
            ...
            .unwrap_or("HashKeyGlobal request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_hashkey_global_error(code.as_deref(), status.as_u16(), message),
            message,
            Utc::now(),
        );
        error.code = code;
...
fn classify_hashkey_global_error(
    code: Option<&str>,
    status: u16,
    message: &str,
) -> ExchangeErrorClass {
