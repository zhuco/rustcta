...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("woo").expect("woo exchange")
}
...
pub(super) fn symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "SPOT_BTC_USDT")
            .expect("symbol"),
    }
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "PERP_BTC_USDT")
            .expect("symbol"),
    }
}

pub(super) fn assert_signed_woo_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert!(!request
        .body
        .as_ref()
        .is_some_and(|body| body.to_string().contains("secret")));
}
