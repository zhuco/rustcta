...
use super::parser::{
    normalize_depth, normalize_woo_symbol, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl WooGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        if request.symbols.len() == 1 {
            params.insert(
                "symbol".to_string(),
                normalize_woo_symbol(&request.symbols[0])?,
            );
        }
        let value = self
            .rest
            .send_public_request("/v3/public/instruments", &params)
            .await?;
        let requested = request
            .symbols
            .iter()
            .map(normalize_woo_symbol)
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let value = self
            .rest
            .send_public_request("/v3/public/orderbook", &params)
            .await?;
