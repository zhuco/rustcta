...
#[derive(Clone)]
pub struct ZetaMarketsRest {
    exchange_id: ExchangeId,
    rest_base_url: String,
    http: reqwest::Client,
}

impl ZetaMarketsRest {
    pub fn new(
        exchange_id: ExchangeId,
        rest_base_url: String,
    ...
    pub fn symbols_path() -> &'static str {
        "/prices/symbols"
    }

    pub fn orderbook_path(symbol: &str) -> String {
        format!("/v2/orderbook?ticker_id={}", sanitize_query_symbol(symbol))
    }
...
fn sanitize_query_symbol(symbol: &str) -> String {
    symbol
        .trim()
        .chars()
