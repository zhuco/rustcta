...
pub use config::ZetaMarketsGatewayConfig;
use transport::ZetaMarketsRest;

#[derive(Clone)]
pub struct ZetaMarketsGatewayAdapter {
    exchange_id: ExchangeId,
    config: ZetaMarketsGatewayConfig,
    rest: ZetaMarketsRest,
}

impl ZetaMarketsGatewayAdapter {
    pub fn new(config: ZetaMarketsGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("zeta_markets").map_err(validation_error)?;
        let rest = ZetaMarketsRest::new(
            exchange_id.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "zeta_markets adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "zeta_markets.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for ZetaMarketsGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "zeta_markets scan-only: legacy public REST data API, venue ceased operations in May 2025, private trading unsupported"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for ZetaMarketsGatewayAdapter {
