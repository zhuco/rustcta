...
pub(super) fn apply_toolchain_capabilities(capabilities: &mut ExchangeClientCapabilities) {
    ...
    capabilities.capabilities_v2.private_rest = CapabilitySupport::unsupported(
        "Zeta Markets ceased operations in May 2025; Solana margin-account reads stay audit-only",
    );
    capabilities.capabilities_v2.public_streams = CapabilitySupport::unsupported(
        "Zeta Markets SDK used Solana account polling/websocket callbacks, not a stable gateway WS API",
    );
    capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
        "Zeta Markets private streams require wallet-owned Solana account subscriptions and are not runtime mapped",
    );
    capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
        public: CapabilitySupport::unsupported(
            "public stream runtime unsupported; use REST data API reconciliation",
        ),
        private: CapabilitySupport::unsupported("private stream runtime unsupported"),
...
fn endpoint_capabilities() -> Vec<EndpointCapability> {
    let mut endpoints = vec![
        EndpointCapability {
            ...
            path: Some("/prices/symbols".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("zeta_markets_public".to_string()),
            weight: Some(1),
        ...
        EndpointCapability {
            ...
            transport: EndpointTransport::Rest,
            method: Some("GET".to_string()),
            path: Some("/v2/orderbook?ticker_id={symbol}".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("zeta_markets_public".to_string()),
            weight: Some(1),
    ...
    endpoints.extend(
        ...
        .map(|(operation, reason)| EndpointCapability {
            ...
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("zeta_markets_unsupported".to_string()),
            weight: Some(0),
            supports_testnet: false,
        }),
    );
