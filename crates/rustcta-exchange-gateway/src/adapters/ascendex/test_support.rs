...
use super::AscendexGatewayConfig;
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("ascendex").expect("ascendex exchange")
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    symbol_scope(MarketType::Perpetual, "BTC-PERP")
}
...
pub(super) fn private_config(rest_base_url: String) -> AscendexGatewayConfig {
...
pub(super) fn fixture_json(name: &str) -> Value {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/ascendex/{name}",
        env!("CARGO_MANIFEST_DIR")
    );
    let text = std::fs::read_to_string(path).expect("AscendEX fixture should be readable");
    serde_json::from_str(&text).expect("AscendEX fixture should be valid JSON")
}
