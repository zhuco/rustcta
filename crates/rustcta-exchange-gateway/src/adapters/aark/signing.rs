...
pub const AARK_ORDERLY_SIGNING_BOUNDARY: &str =
    "aark uses Orderly Ed25519 account headers; private read REST requires explicit account id, orderly key and base58/hex orderly secret";
...
pub fn aark_orderly_canonical_payload(
    timestamp_ms: i64,
    method: &str,
    path_with_query: &str,
    body: &str,
...
pub fn aark_orderly_signing_boundary() -> &'static str {
    AARK_ORDERLY_SIGNING_BOUNDARY
}
...
pub fn sign_orderly_request(
    ...
    let canonical_payload =
        aark_orderly_canonical_payload(timestamp_ms, method, path_with_query, body);
...
fn decode_orderly_secret(secret: &str) -> ExchangeApiResult<[u8; 32]> {
    ...
    bytes
        ...
        .map_err(|bytes: Vec<u8>| ExchangeApiError::InvalidRequest {
            message: format!(
                "aark orderly secret must decode to 32 bytes, got {} bytes",
                bytes.len()
            ),
        })
}
...
fn decode_base58(value: &str) -> ExchangeApiResult<Vec<u8>> {
    ...
    if value.trim().is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "aark orderly secret must not be empty".to_string(),
        });
    }
    ...
    for ch in value.bytes() {
        let carry = ALPHABET
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "invalid base58 character in aark orderly secret".to_string(),
            })? as u32;
        let mut carry = carry;
