...
use rustcta_exchange_api::{
    ...
    ExchangeClient, ExchangeClientCapabilities, FeesRequest, FeesResponse, OpenOrdersRequest,
    OpenOrdersResponse, OrderBookCapability, OrderBookRequest, OrderBookResponse,
    OrderBookStrictness, OrderListRequest, OrderListResponse, OrderType, PlaceOrderRequest,
...
pub use config::BydfiGatewayConfig;
use transport::BydfiRest;

#[derive(Clone)]
pub struct BydfiGatewayAdapter {
    exchange_id: ExchangeId,
    config: BydfiGatewayConfig,
    rest: BydfiRest,
}

impl BydfiGatewayAdapter {
    pub fn new(config: BydfiGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("bydfi").map_err(validation_error)?;
        let rest = BydfiRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BydfiGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("BYDFi adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for BydfiGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "BYDFi USDT perpetual public/private REST gateway adapter"
                } else {
                    "BYDFi USDT perpetual public REST gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for BydfiGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.order_book = OrderBookCapability {
            ...
            checksum_mode: rustcta_exchange_api::OrderBookChecksumMode::Disabled,
        ...
        capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
            "BYDFi documents private WS login/events but not a stable connection URL",
        );
    ...
    async fn place_order_list(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "bydfi.place_order_list",
        })
    }
