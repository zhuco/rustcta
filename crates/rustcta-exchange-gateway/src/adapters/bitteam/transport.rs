...
async fn parse_response(
    ...
    let ok_flag = value.get("ok");
    let failed_ok = matches!(ok_flag, Some(Value::Bool(false)))
        || ok_flag
    ...
    if !status.is_success() || failed_ok {
        let message = value
            ...
            .and_then(Value::as_str)
            .unwrap_or("BIT.TEAM request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_bitteam_error(message, status.as_u16()),
            message,
            Utc::now(),
        );
        error.code = value
...
fn classify_bitteam_error(message: &str, status: u16) -> ExchangeErrorClass {
