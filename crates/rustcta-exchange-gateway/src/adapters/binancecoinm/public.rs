...
use super::parser::{
    normalize_binancecoinm_symbol, normalize_depth, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl BinanceCoinMGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let requested = request
            .symbols
            .iter()
            .map(|symbol| normalize_binancecoinm_symbol(&symbol.exchange_symbol.symbol))
        ...
        let mut response = self
            .rest
            .send_public_request("/dapi/v1/exchangeInfo", &params)
            .await?;
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let value = self
            .rest
            .send_public_request("/dapi/v1/depth", &params)
            .await?;
...
fn retain_requested_symbols(response: &mut Value, requested: &[String]) {
    ...
    symbols.retain(|symbol| {
        symbol
            ...
            .and_then(|symbol| normalize_binancecoinm_symbol(symbol).ok())
