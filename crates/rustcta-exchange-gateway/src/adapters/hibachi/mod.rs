...
pub use config::HibachiGatewayConfig;
use transport::HibachiRest;

#[derive(Clone)]
pub struct HibachiGatewayAdapter {
    exchange_id: ExchangeId,
    config: HibachiGatewayConfig,
    rest: HibachiRest,
}

impl HibachiGatewayAdapter {
    pub fn new(config: HibachiGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("hibachi").map_err(validation_error)?;
        let rest = HibachiRest::new(
            exchange_id.clone(),
            config.data_rest_base_url.clone(),
            config.account_rest_base_url.clone(),
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("hibachi adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Perpetual {
            return Err(ExchangeApiError::Unsupported {
                operation: "hibachi.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for HibachiGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "hibachi perpetual public REST enabled; private writes are request-spec only"
                    .to_string(),
            ),
        }
    }
...
#[async_trait]
impl ExchangeClient for HibachiGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.order_book = OrderBookCapability::snapshot_only(Some(500));
    ...
    async fn place_order(
        ...
        private::write_boundary(private::PLACE_ORDER_UNSUPPORTED)
    }
    ...
    async fn place_quote_market_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_supported_market_type(request.symbol.market_type)?;
        private::write_boundary(private::QUOTE_MARKET_UNSUPPORTED)
    }
    ...
    async fn cancel_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_supported_market_type(request.symbol.market_type)?;
        private::write_boundary(private::CANCEL_ORDER_UNSUPPORTED)
    }
    ...
    async fn amend_order(
        ...
        self.ensure_exchange(&request.symbol.exchange)?;
        self.ensure_supported_market_type(request.symbol.market_type)?;
        private::write_boundary(private::AMEND_ORDER_UNSUPPORTED)
    }
    ...
    async fn place_order_list(
        ...
        self.ensure_exchange(&request.symbol().exchange)?;
        self.ensure_supported_market_type(request.symbol().market_type)?;
        private::write_boundary(private::ORDER_LIST_UNSUPPORTED)
    }
    ...
    async fn batch_place_orders(
        ...
        private::write_boundary(private::BATCH_PLACE_UNSUPPORTED)
    }
    ...
    async fn batch_cancel_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        private::write_boundary(private::BATCH_CANCEL_UNSUPPORTED)
    }
    ...
    async fn cancel_all_orders(
        ...
        self.ensure_exchange(&request.exchange)?;
        self.ensure_optional_market_type(request.market_type)?;
        private::write_boundary(private::CANCEL_ALL_UNSUPPORTED)
    }
