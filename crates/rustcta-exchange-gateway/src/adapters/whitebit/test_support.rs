...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("whitebit").expect("whitebit exchange")
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "BTC_PERP")
            .expect("symbol"),
    }
}

pub(super) fn assert_signed_request(request: &SeenRequest, api_key: &str, api_secret: &str) {
    assert_signed_request_method(request, "POST", api_key, api_secret);
}
...
pub(super) fn assert_signed_request_method(
    ...
    assert_eq!(request.header("x-txc-apikey"), Some(api_key));
    let payload = request.header("x-txc-payload").expect("payload header");
    assert!(request.header("x-txc-signature").is_some());
    let decoded = base64::engine::general_purpose::STANDARD
        .decode(payload)
        .expect("valid payload base64");
    let decoded: Value = serde_json::from_slice(&decoded).expect("payload json");
    assert_eq!(request.body.as_ref(), Some(&decoded));
    assert!(request
        .body
        .as_ref()
    ...
    assert!(request
        ...
        .is_some());
    assert!(!request.path.contains(api_secret));
    assert!(!request
        .query
        .values()
        .any(|value| value.contains(api_secret)));
}
