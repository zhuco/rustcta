...
use rustcta_exchange_api::{
    ...
    RecentFillsResponse, ReconnectCapability, StreamAuthCapability, StreamResyncCapability,
...
pub use config::WhiteBitGatewayConfig;
use transport::WhiteBitPublicRest;

#[derive(Clone)]
pub struct WhiteBitGatewayAdapter {
    exchange_id: ExchangeId,
    config: WhiteBitGatewayConfig,
    rest: WhiteBitPublicRest,
}

impl WhiteBitGatewayAdapter {
    pub fn new(config: WhiteBitGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("whitebit").map_err(validation_error)?;
        let rest = WhiteBitPublicRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(WhiteBitGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("whitebit adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "whitebit.unsupported_market_type",
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "whitebit.non_spot_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "whitebit private REST request requires tenant_id".to_string(),
                })?;
        let account_id =
            context
                .account_id
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "whitebit private REST request requires account_id".to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for WhiteBitGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("whitebit spot/perpetual REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for WhiteBitGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.supports_reduce_only = self.config.private_rest_enabled();
        ...
        capabilities.private_stream_capabilities =
            Some(rustcta_exchange_api::PrivateStreamCapabilities {
                ...
                order_event_kinds: if self.config.private_rest_enabled() {
                    vec![
                ...
                supports_client_order_id: self.config.private_rest_enabled(),
                supports_exchange_order_id: self.config.private_rest_enabled(),
            });
            ...
            rustcta_exchange_api::StreamRuntimeCapability {
                ...
                private: if private {
                    ...
                    CapabilitySupport::unsupported(
                        "whitebit private streams require API key/secret and websocket token",
                    )
                },
