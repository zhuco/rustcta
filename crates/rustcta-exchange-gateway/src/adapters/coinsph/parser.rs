    ...
    symbols
        ...
        .filter(|value| {
            value
                ...
                .is_some_and(|quote| quote.eq_ignore_ascii_case("PHP"))
        })
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "Coins.ph order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_coinsph_symbol(symbol: &str) -> ExchangeApiResult<String> {
    ...
    if normalized.is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "Coins.ph symbol must not be empty".to_string(),
        });
    }
    Ok(normalized)
}

pub fn ensure_php_symbol(symbol: &SymbolScope, operation: &'static str) -> ExchangeApiResult<()> {
    let is_php = symbol
        .canonical_symbol
        .as_ref()
        .is_some_and(|canonical| canonical.quote_asset() == "PHP")
        || symbol
            .exchange_symbol
            .symbol
            .to_ascii_uppercase()
...
pub(super) fn decimal_text_to_f64(value: &str) -> ExchangeApiResult<f64> {
    value
        ...
        .map_err(|error| ExchangeApiError::InvalidRequest {
            message: format!("invalid Coins.ph decimal value {value}: {error}"),
        })
}
...
pub(super) fn parse_side(side: &str) -> ExchangeApiResult<OrderSide> {
    match side.to_ascii_uppercase().as_str() {
        ...
        _ => Err(ExchangeApiError::InvalidRequest {
            message: format!("unsupported Coins.ph side {side}"),
        }),
    }
}
