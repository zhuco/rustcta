...
async fn parse_response(
    ...
    if !status.is_success() {
        ...
        let message = value
            ...
            .unwrap_or("Coins.ph request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_coinsph_error(code.as_deref(), message),
            message,
            Utc::now(),
        );
        error.code = code;
...
fn classify_coinsph_error(code: Option<&str>, message: &str) -> ExchangeErrorClass {
