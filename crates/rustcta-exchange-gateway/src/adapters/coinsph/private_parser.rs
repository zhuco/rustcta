    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "Coins.ph recent fills request requires canonical_symbol".to_string(),
            })?;
    ...
    fills
        ...
        .map(|fill| {
            ...
            let quantity = decimal_value_to_f64(fill.get("qty").or_else(|| fill.get("quantity")))?
                .unwrap_or(0.0);
            let quote_quantity =
                decimal_value_to_f64(fill.get("quoteQty").or_else(|| fill.get("quoteQuantity")))?;
            let fee_amount =
                decimal_value_to_f64(fill.get("commission").or_else(|| fill.get("fee")))?;
            Ok(Fill {
                schema_version: SchemaVersion::current(),
                ...
                fill_id: value_as_string(fill.get("id").or_else(|| fill.get("tradeId"))),
                side: if is_buyer {
                    OrderSide::Buy
                } else {
                    OrderSide::Sell
                ...
                fee_asset: value_as_string(
                    fill.get("commissionAsset").or_else(|| fill.get("feeAsset")),
                ),
                fee_amount,
                fee_rate: None,
                realized_pnl: None,
...
pub fn parse_fee_snapshots(
    ...
    array
        ...
        .map(|item| {
            let symbol = if let Some(symbol) = requested_symbols.first() {
                ...
                SymbolScope {
                    ...
                    exchange_symbol: ExchangeSymbol::new(
                        exchange_id.clone(),
                        MarketType::Spot,
                        required_str(exchange_id, item, "symbol")?.to_ascii_uppercase(),
                    )
                    .map_err(validation_error)?,
                }
            };
            ...
            .ok_or_else(|| {
                parse_error(exchange_id.clone(), "fee response missing maker rate", item)
            })?;
            ...
            .ok_or_else(|| {
                parse_error(exchange_id.clone(), "fee response missing taker rate", item)
            })?;
            Ok(FeeRateSnapshot {
                schema_version: EXCHANGE_API_SCHEMA_VERSION,
                symbol,
                maker_rate: maker,
                taker_rate: taker,
                source: Some("coinsph.asset_trade_fee".to_string()),
                updated_at: Utc::now(),
            })
        })
        .collect()
...
fn map_coinsph_order_status(status: &str) -> OrderStatus {
