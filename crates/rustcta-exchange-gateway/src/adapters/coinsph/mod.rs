...
pub use config::CoinsPhGatewayConfig;
use transport::CoinsPhRest;

#[derive(Clone)]
pub struct CoinsPhGatewayAdapter {
    exchange_id: ExchangeId,
    config: CoinsPhGatewayConfig,
    rest: CoinsPhRest,
}

impl CoinsPhGatewayAdapter {
    pub fn new(config: CoinsPhGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("coinsph").map_err(validation_error)?;
        let rest = CoinsPhRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(CoinsPhGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("Coins.ph adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "coinsph.non_spot_market_type",
            });
        }
        Ok(())
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "Coins.ph private REST readback requires context.tenant_id"
                        .to_string(),
                })?;
        let account_id =
            context
                ...
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "Coins.ph private REST readback requires context.account_id"
                        .to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for CoinsPhGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("Coins.ph Philippines spot REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for CoinsPhGatewayAdapter {
    ...
    async fn get_positions(
        ...
        self.unsupported_private("coinsph.get_positions")
    }
    ...
    async fn place_quote_market_order(
        ...
        self.unsupported_private("coinsph.place_quote_market_order")
    }
    ...
    async fn amend_order(
        ...
        self.unsupported_private("coinsph.amend_order")
    }
    ...
    async fn place_order_list(
        ...
        self.unsupported_private("coinsph.place_order_list")
    }
    ...
    async fn subscribe_private_stream(
        ...
        self.unsupported_private("coinsph.subscribe_private_stream")
    }
}
