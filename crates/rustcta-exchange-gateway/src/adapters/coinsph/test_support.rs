...
use super::CoinsPhGatewayConfig;
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("coinsph").expect("coinsph exchange")
}
...
pub(super) fn symbol_scope(exchange_symbol: &str) -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "PHP").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, exchange_symbol)
            .expect("symbol"),
    }
}

pub(super) fn usdt_symbol_scope(exchange_symbol: &str) -> SymbolScope {
    SymbolScope {
        exchange: exchange_id(),
        market_type: MarketType::Spot,
...
pub(super) fn private_config(base_url: String) -> CoinsPhGatewayConfig {
    CoinsPhGatewayConfig {
        rest_base_url: base_url,
        api_key: Some("coins-test-key".to_string()),
        api_secret: Some("coins-test-secret".to_string()),
...
pub(super) fn assert_signed_request_method(request: &SeenRequest, method: &str) {
    ...
    assert_eq!(request.header("X-COINS-APIKEY"), Some("coins-test-key"));
    ...
    assert!(!request
        .query
        .values()
        .any(|value| value.contains("coins-test-secret")));
}
