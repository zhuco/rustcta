    ...
    tokio::spawn(async move {
        loop {
            ...
            let body = responses
                ...
                .unwrap()
                .next()
                .unwrap_or_else(|| json!({ "code": 1000, "data": {} }));
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("bitmart").expect("bitmart exchange")
}
...
pub(super) fn spot_symbol_scope() -> SymbolScope {
    symbol_scope(MarketType::Spot, "BTC_USDT")
}

pub(super) fn perp_symbol_scope() -> SymbolScope {
    symbol_scope(MarketType::Perpetual, "BTCUSDT")
}

fn symbol_scope(market_type: MarketType, exchange_symbol: &str) -> SymbolScope {
    SymbolScope {
        exchange: exchange_id(),
        market_type,
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), market_type, exchange_symbol)
            .expect("symbol"),
    }
}

pub(super) fn private_adapter(base_url: String) -> super::BitmartGatewayAdapter {
    super::BitmartGatewayAdapter::new(super::BitmartGatewayConfig {
        ...
        ..super::BitmartGatewayConfig::default()
    })
    .expect("adapter")
}
...
pub(super) fn assert_signed_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert_eq!(
        request.headers.get("x-bm-key").map(String::as_str),
        Some("key")
    );
    assert_eq!(
        request.headers.get("x-bm-broker-id").map(String::as_str),
        Some("memo")
    );
    assert!(request
        .headers
    ...
    assert!(request
        .headers
        .get("x-bm-sign")
        .is_some_and(|signature| !signature.contains("secret") && signature.len() == 64));
}
