...
use super::parser::{
    normalize_depth, normalize_symbol, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl BitmartGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        for market_type in markets {
            let endpoint = match market_type {
                MarketType::Spot => "/spot/v1/symbols/details",
                MarketType::Perpetual => "/contract/public/details",
                _ => unreachable!("checked by ensure_supported_market"),
            };
            let response = self
                .rest
                .send_public_get(market_type, endpoint, &HashMap::new())
                .await?;
    ...
    pub(super) async fn get_order_book_impl(
        ...
        params.insert("size".to_string(), normalize_depth(request.depth));
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => "/spot/v1/symbols/book",
            MarketType::Perpetual => "/contract/public/depth",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        let value = self
            .rest
            .send_public_get(request.symbol.market_type, endpoint, &params)
            .await?;
