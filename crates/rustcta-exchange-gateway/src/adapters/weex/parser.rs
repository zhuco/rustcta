...
fn parse_symbol_rule(
    ...
    let (fallback_base, fallback_quote) = split_weex_symbol(&exchange_symbol)
    ...
    let tradable = !matches!(
        ...
        "HALT" | "SUSPEND" | "OFFLINE" | "CLOSE" | "DISABLED"
    );
    ...
    Ok(SymbolRules {
        ...
        .or_else(|| string_or_number(value.get("minTradeAmount")))
        .or_else(|| string_or_number(value.get("minOrderSize"))),
        max_quantity: string_or_number(
            value
                .get("maxQty")
                .or_else(|| value.get("maxTradeQty"))
                .or_else(|| value.get("maxTradeAmount"))
                .or_else(|| value.get("maxOrderSize")),
        ),
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "weex order book request requires canonical_symbol".to_string(),
            })?;
...
pub fn normalize_weex_symbol(symbol: &str, market_type: MarketType) -> ExchangeApiResult<String> {
    ...
    let (base, quote) =
        split_compact_symbol(&compact).ok_or_else(|| ExchangeApiError::InvalidRequest {
            message: format!("cannot normalize WEEX symbol {symbol}"),
        })?;
    let _ = market_type;
...
pub(super) fn split_weex_symbol(symbol: &str) -> Option<(String, String)> {
