...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("weex").expect("weex exchange")
}
...
pub(super) fn assert_signed_weex_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert!(!request.path.contains("secret"));
...
pub(super) fn request_spec(name: &str) -> crate::request_spec::RequestSpec {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/weex/request_specs/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("WEEX request spec fixture"))
        .expect("WEEX request spec json")
}

pub(super) fn signing_vector(name: &str) -> crate::signing_spec::SigningVector {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/weex/signing_vectors/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("WEEX signing vector fixture"))
        .expect("WEEX signing vector json")
}
