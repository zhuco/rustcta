...
pub(super) fn apply_toolchain_capabilities(capabilities: &mut ExchangeClientCapabilities) {
    ...
    capabilities.capabilities_v2.private_rest = CapabilitySupport::unsupported(
        "Aftermath account reads and writes require Sui wallet/account-cap semantics",
    );
    capabilities.capabilities_v2.public_streams = CapabilitySupport::unsupported(
        "Aftermath public WebSocket payloads are spec-only until runtime resync is implemented",
    );
    capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
        "Aftermath private streams require Sui account-scoped authorization",
    );
    capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
        public: CapabilitySupport::unsupported(
            "Aftermath public WS is payload/parser-only in this adapter",
        ),
        private: CapabilitySupport::unsupported("Aftermath private WS auth is not mapped"),
...
fn endpoint_capabilities() -> Vec<EndpointCapability> {
    let mut endpoints = vec![
        EndpointCapability {
            ...
            path: Some("/api/ccxt/markets".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("aftermath_public".to_string()),
            weight: Some(1),
        ...
        EndpointCapability {
            ...
            path: Some("/api/ccxt/orderbook".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("aftermath_public".to_string()),
            weight: Some(1),
    ...
    endpoints.extend(
        ...
        .map(|(operation, reason)| EndpointCapability {
            ...
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("aftermath_unsupported".to_string()),
            weight: Some(0),
