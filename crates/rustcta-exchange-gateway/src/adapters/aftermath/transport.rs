...
impl AftermathRest {
    ...
    pub async fn send_public_get(&self, path: &str) -> ExchangeApiResult<Value> {
        let response = self
            .with_optional_auth(self.http.get(self.url(path)))
            .send()
            .await
            .map_err(|error| ExchangeApiError::Transport {
                message: error.to_string(),
    ...
    pub async fn send_public_post(&self, path: &str, body: &Value) -> ExchangeApiResult<Value> {
        let response = self
            .with_optional_auth(self.http.post(self.url(path)).json(body))
            .send()
            .await
            .map_err(|error| ExchangeApiError::Transport {
                message: error.to_string(),
