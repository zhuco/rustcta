...
pub fn exchange_id() -> ExchangeId {
    ExchangeId::new("bigone").expect("exchange")
}
...
impl SeenRequest {
    ...
    pub(super) fn raw_contains_secret(&self) -> bool {
        self.headers
            ...
            || self.body.contains("test-secret")
            || self.path.contains("test-secret")
            || self
                .query
                .values()
                .any(|value| value.contains("test-secret"))
    ...
    pub(super) fn actual_http_request(&self) -> crate::request_spec::ActualHttpRequest {
        crate::request_spec::ActualHttpRequest::new(self.method.clone(), self.path.clone())
            ...
            .with_body(if self.body.trim().is_empty() {
                None
            } else {
...
pub fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        exchange: exchange_id(),
...
pub(super) fn private_config(base_url: String) -> super::BigOneGatewayConfig {
    super::BigOneGatewayConfig {
        spot_rest_base_url: base_url.clone(),
        contract_rest_base_url: base_url,
        api_key: Some("test-key".to_string()),
        api_secret: Some("test-secret".to_string()),
        enabled_private_rest: true,
        ..super::BigOneGatewayConfig::default()
    }
}
