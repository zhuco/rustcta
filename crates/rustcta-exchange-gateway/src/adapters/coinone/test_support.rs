    ...
    tokio::spawn(async move {
        loop {
            ...
            let body = responses
                ...
                .unwrap()
                .next()
                .unwrap_or_else(|| json!({ "result": "success", "error_code": "0" }));
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("coinone").expect("coinone exchange")
}
...
pub(super) fn symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "BTC-KRW")
            .expect("symbol"),
    }
}
