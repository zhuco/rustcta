...
pub use config::CoinwGatewayConfig;
use signing::CoinwPrivateCredentials;
use transport::CoinwRest;

#[derive(Clone)]
pub struct CoinwGatewayAdapter {
    exchange_id: ExchangeId,
    config: CoinwGatewayConfig,
    rest: CoinwRest,
}

impl CoinwGatewayAdapter {
    pub fn new(config: CoinwGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("coinw").map_err(validation_error)?;
        let credentials = CoinwPrivateCredentials::from_config(&config);
        let rest = CoinwRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(CoinwGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("coinw adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for CoinwGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("coinw spot + perpetual public REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for CoinwGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = if private {
            Some(streams::coinw_private_stream_capabilities())
        } else {
