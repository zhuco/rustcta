...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("bitopro").expect("bitopro exchange")
}
...
pub(super) fn symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "TWD").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "btc_twd")
            .expect("symbol"),
    }
}

pub(super) fn assert_bitopro_signed_request(request: &SeenRequest, api_key: &str) {
    assert_eq!(request.header("x-bitopro-apikey"), Some(api_key));
    assert!(request.header("x-bitopro-payload").is_some());
    assert!(request.header("x-bitopro-signature").is_some());
}
