...
use super::parser::{
    normalize_bitopro_pair, normalize_depth, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl BitoproGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let value = self
            .rest
            .send_public_request("/provisioning/trading-pairs", &HashMap::new())
            .await?;
        ...
        if !request.symbols.is_empty() {
            let requested = request
                .symbols
                .iter()
                .map(|symbol| normalize_bitopro_pair(&symbol.exchange_symbol.symbol))
    ...
    pub(super) async fn get_order_book_impl(
        ...
        params.insert(
            "limit".to_string(),
            normalize_depth(request.depth).to_string(),
        );
        params.insert("scale".to_string(), "0".to_string());
        let value = self
            .rest
            .send_public_request(&format!("/order-book/{pair}"), &params)
            .await?;
