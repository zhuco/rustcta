...
async fn parse_response(
    ...
    let failed_ok = matches!(success_flag, Some(Value::Bool(false)))
        ...
        || success_flag
            ...
            .is_some_and(|text| text.eq_ignore_ascii_case("false") || text == "0");
    if !status.is_success() || failed_ok {
        let message = value
            ...
            .and_then(Value::as_str)
            .unwrap_or("YOBIT request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_yobit_error(message, status.as_u16()),
            message,
            Utc::now(),
        );
        error.code = value
...
fn classify_yobit_error(message: &str, status: u16) -> ExchangeErrorClass {
