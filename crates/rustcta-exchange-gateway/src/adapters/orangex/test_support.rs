    ...
    tokio::spawn(async move {
        loop {
            ...
            let body = responses
                ...
                .unwrap()
                .next()
                .unwrap_or_else(|| json!({ "jsonrpc": "2.0", "id": "1", "result": {} }));
...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("orangex").expect("orangex exchange")
}
...
pub(super) fn spot_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Spot, "BTC-USDT-SPOT")
            .expect("symbol"),
    }
}
...
pub(super) fn request_spec(name: &str) -> crate::request_spec::RequestSpec {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/orangex/request_specs/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("orangex request spec fixture"))
        .expect("orangex request spec json")
}

pub(super) fn signing_vector(name: &str) -> crate::signing_spec::SigningVector {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/orangex/signing_vectors/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("orangex signing vector fixture"))
        .expect("orangex signing vector json")
}
