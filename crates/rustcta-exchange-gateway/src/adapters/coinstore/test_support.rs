...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("coinstore").expect("exchange")
}
...
pub(super) fn perp_symbol_scope() -> SymbolScope {
    SymbolScope {
        ...
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(exchange_id(), MarketType::Perpetual, "12345")
            .expect("symbol"),
    }
}

pub(super) fn private_config(base_url: String) -> super::CoinstoreGatewayConfig {
    super::CoinstoreGatewayConfig {
        spot_rest_base_url: base_url.clone(),
        futures_rest_base_url: base_url,
        api_key: Some("key".to_string()),
        api_secret: Some("secret".to_string()),
        enabled_private_rest: true,
        ..super::CoinstoreGatewayConfig::default()
    }
}
...
pub(super) fn assert_signed_request(request: &SeenRequest, method: &str, path: &str) {
    ...
    assert_eq!(
        request.headers.get("x-cs-apikey").map(String::as_str),
        Some("key")
    );
    assert!(request
        .headers
