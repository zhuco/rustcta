...
use super::parser::{
    ...
    parse_side, split_bitrue_symbol, string_or_number, validation_error, value_as_string,
};
...
pub fn parse_positions(
    ...
    for position in positions {
        let symbol = position
            .get("symbol")
            .or_else(|| position.get("contractName"))
            .and_then(Value::as_str)
            .ok_or_else(|| parse_error(exchange_id.clone(), "position missing symbol", position))?
            .to_ascii_uppercase();
        let (base, quote) = split_bitrue_symbol(&symbol)
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "bitrue fill requires canonical_symbol".to_string(),
            })?;
...
fn parse_fee_snapshot(
    ...
    FeeRateSnapshot {
        ...
        source: Some(if market_type == MarketType::Spot {
            "bitrue.spot.account_commission".to_string()
        } else {
            "bitrue.futures.commission_rate".to_string()
        }),
        updated_at: Utc::now(),
    }
}
...
fn map_bitrue_order_status(status: &str) -> OrderStatus {
