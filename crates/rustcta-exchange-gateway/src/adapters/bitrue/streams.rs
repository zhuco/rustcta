...
use super::parser::{
    normalize_bitrue_symbol, parse_error, parse_orderbook_snapshot, split_bitrue_symbol,
    validation_error,
};
...
pub enum BitruePublicStreamMessage {
    OrderBook(OrderBookSnapshot),
    Trades(Vec<BitruePublicTrade>),
    SubscriptionAck { id: Option<String> },
    Pong,
}

#[derive(Debug, Clone, PartialEq)]
pub enum BitruePrivateStreamMessage {
...
#[derive(Debug, Clone, PartialEq)]
pub enum BitrueWsSessionEvent {
    Public(BitruePublicStreamMessage),
    Private(BitruePrivateStreamMessage),
    Stream(ExchangeStreamEvent),
    HeartbeatResponse(Value),
}

#[derive(Debug, Clone)]
pub struct BitruePublicWsSession {
    pub url: String,
...
#[derive(Debug, Clone)]
pub struct BitruePrivateWsSession {
    pub url: String,
    exchange_id: ExchangeId,
    tenant_id: TenantId,
...
impl BitrueGatewayAdapter {
    pub(super) async fn subscribe_public_stream_impl(
        ...
        let payload = bitrue_public_subscribe_payload(&subscription, "1")?;
        Ok(format!(
            "bitrue:{}:{}",
            self.config.spot_public_ws_url,
            bitrue_public_payload_channel(&payload).unwrap_or("unknown")
        ))
    }
    ...
    pub(super) async fn subscribe_private_stream_impl(
        ...
        let channel = bitrue_private_channel(&subscription.kind, market_type)?;
    ...
    pub async fn create_private_stream_listen_key(
        &self,
        market_type: MarketType,
    ) -> ExchangeApiResult<BitrueListenKey> {
        self.ensure_supported_market(market_type)?;
        let (api_key, api_secret, recv_window_ms) =
            self.private_credentials("bitrue.create_private_stream_listen_key")?;
        let value = match market_type {
            MarketType::Spot => {
                self.rest
                    .send_spot_api_key_request(
                        reqwest::Method::POST,
                        BITRUE_SPOT_LISTEN_KEY_ENDPOINT,
                        api_key,
                    )
                    .await?
            }
            MarketType::Perpetual => {
                self.rest
                    .send_futures_ws_auth_signed_request(
                        reqwest::Method::POST,
                        BITRUE_FUTURES_LISTEN_KEY_ENDPOINT,
                        &json!({}),
                        api_key,
                        api_secret,
                        recv_window_ms,
        ...
        parse_bitrue_listen_key(&self.config, market_type, &value)
    }

    pub async fn keepalive_private_stream_listen_key(
        &self,
        market_type: MarketType,
        listen_key: &str,
    ) -> ExchangeApiResult<BitrueListenKey> {
        self.ensure_supported_market(market_type)?;
        let listen_key = normalized_listen_key(listen_key)?;
        let (api_key, api_secret, recv_window_ms) =
            self.private_credentials("bitrue.keepalive_private_stream_listen_key")?;
        match market_type {
            MarketType::Spot => {
                self.rest
                    .send_spot_api_key_request(
                        reqwest::Method::PUT,
                        &format!("{BITRUE_SPOT_LISTEN_KEY_ENDPOINT}/{listen_key}"),
                        api_key,
                    )
                    .await?;
            }
            MarketType::Perpetual => {
                self.rest
                    .send_futures_ws_auth_signed_request(
                        reqwest::Method::PUT,
                        &format!("{BITRUE_FUTURES_LISTEN_KEY_ENDPOINT}/{listen_key}"),
                        &json!({}),
                        api_key,
                        api_secret,
                        recv_window_ms,
        ...
        bitrue_listen_key_from_text(&self.config, market_type, listen_key)
    }

    pub async fn close_private_stream_listen_key(
        &self,
        market_type: MarketType,
        listen_key: &str,
    ) -> ExchangeApiResult<BitrueListenKey> {
        ...
        let listen_key = normalized_listen_key(listen_key)?;
        let (api_key, api_secret, recv_window_ms) =
            self.private_credentials("bitrue.close_private_stream_listen_key")?;
        match market_type {
            MarketType::Spot => {
                self.rest
                    .send_spot_api_key_request(
...
const BITRUE_SPOT_LISTEN_KEY_ENDPOINT: &str = "/poseidon/api/v1/listenKey";
const BITRUE_FUTURES_LISTEN_KEY_ENDPOINT: &str = "/user_stream/api/v1/listenKey";

pub fn bitrue_public_subscribe_payload(
    subscription: &PublicStreamSubscription,
    id: &str,
) -> ExchangeApiResult<Value> {
    ensure_exchange_api_schema(subscription.schema_version)?;
    let symbol = bitrue_ws_symbol(
...
fn bitrue_public_payload_channel(payload: &Value) -> Option<&str> {
    payload
        .get("params")
...
pub fn bitrue_private_subscribe_payload(
    subscription: &PrivateStreamSubscription,
    market_type: MarketType,
) -> ExchangeApiResult<Value> {
    ensure_exchange_api_schema(subscription.schema_version)?;
...
pub fn bitrue_private_listen_key_url(
    base_url: &str,
    listen_key: &str,
    market_type: MarketType,
) -> ExchangeApiResult<String> {
    ...
    if listen_key.is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "bitrue private stream requires listenKey".to_string(),
        });
    }
    let suffix = match market_type {
        ...
        _ => {
            return Err(ExchangeApiError::Unsupported {
                operation: "bitrue.private_stream_market_type",
            });
        }
    };
...
pub fn parse_bitrue_listen_key(
    ...
    let listen_key = value
        ...
        .or_else(|| value.get("listenKey"))
        .and_then(Value::as_str)
        .ok_or_else(|| {
            parse_error(
                ExchangeId::new("bitrue").expect("static bitrue exchange id"),
                "bitrue listenKey response missing listenKey",
                value,
            )
        })?
        .to_string();
    bitrue_listen_key_from_text(config, market_type, listen_key)
}

fn bitrue_listen_key_from_text(
    config: &BitrueGatewayConfig,
    market_type: MarketType,
    listen_key: String,
) -> ExchangeApiResult<BitrueListenKey> {
    let base_url = match market_type {
        MarketType::Spot => &config.spot_private_ws_url,
        MarketType::Perpetual => &config.futures_private_ws_url,
        _ => {
            return Err(ExchangeApiError::Unsupported {
                operation: "bitrue.private_stream_market_type",
            });
    ...
    Ok(BitrueListenKey {
        websocket_url: bitrue_private_listen_key_url(base_url, &listen_key, market_type)?,
        listen_key,
        market_type,
        expires_after_minutes: if market_type == MarketType::Spot {
            60
        } else {
            30
        },
...
fn normalized_listen_key(listen_key: &str) -> ExchangeApiResult<String> {
    let listen_key = listen_key.trim();
    if listen_key.is_empty() {
        return Err(ExchangeApiError::InvalidRequest {
            message: "bitrue listenKey must not be empty".to_string(),
        });
    }
    Ok(listen_key.to_string())
}

pub fn bitrue_ws_pong_payload(ts: Option<&str>) -> Value {
    match ts {
...
pub fn parse_bitrue_public_stream_message(
    exchange_id: &ExchangeId,
    symbol_hint: SymbolScope,
    value: &Value,
) -> ExchangeApiResult<BitruePublicStreamMessage> {
    if value
        .as_str()
    ...
    {
        return Ok(BitruePublicStreamMessage::OrderBook(
    ...
    {
        return Ok(BitruePublicStreamMessage::Trades(vec![parse_public_trade(
            exchange_id,
            symbol_hint,
            data,
        )?]));
    ...
    Err(parse_error(
        exchange_id.clone(),
        "unsupported bitrue public stream message",
        value,
    ))
}

pub fn parse_bitrue_private_stream_message(
    exchange_id: &ExchangeId,
    tenant_id: TenantId,
    account_id: AccountId,
    market_type: MarketType,
    symbol_hint: Option<SymbolScope>,
    value: &Value,
) -> ExchangeApiResult<BitruePrivateStreamMessage> {
    if value
        .get("event")
        .and_then(Value::as_str)
    ...
    {
        return Ok(BitruePrivateStreamMessage::Pong);
    }
    if value
        .get("status")
        .and_then(Value::as_str)
        .is_some_and(|status| status.eq_ignore_ascii_case("ok"))
    {
        return Ok(BitruePrivateStreamMessage::SubscriptionAck {
    ...
    if event.contains("order_trade_update") {
        let order_data = order_event_payload(value, data);
        ...
        return Ok(BitruePrivateStreamMessage::Events(events));
    }
    if event.contains("account_update") {
        if data.get("a").is_none() && looks_like_position_payload(data) {
            return Ok(BitruePrivateStreamMessage::Events(vec![
    ...
    Err(parse_error(
        exchange_id.clone(),
        "unsupported bitrue private stream message",
        value,
    ))
}
...
fn bitrue_private_channel(
    ...
    match (kind, market_type) {
        ...
        (PrivateStreamKind::Positions, MarketType::Spot) => Err(ExchangeApiError::Unsupported {
            operation: "bitrue.spot_positions_stream",
        }),
        _ => Err(ExchangeApiError::Unsupported {
            operation: "bitrue.private_stream_kind",
        }),
    }
}

impl BitruePublicWsSession {
    pub fn new(
        exchange_id: ExchangeId,
        url: String,
        subscription: PublicStreamSubscription,
        ...
        ensure_exchange_api_schema(subscription.schema_version)?;
        let subscribe_payload = bitrue_public_subscribe_payload(&subscription, "1")?;
        let mut state =
    ...
    pub fn heartbeat_response(&mut self, value: &Value, now: DateTime<Utc>) -> Option<Value> {
        ...
        if is_ping {
            ...
            Some(bitrue_ws_pong_payload(ts))
        } else {
            None
        }
    }
    ...
    pub fn handle_text_message(
        &mut self,
        text: &str,
    ) -> ExchangeApiResult<Vec<BitrueWsSessionEvent>> {
        let value = parse_ws_text(&self.exchange_id, text)?;
        let now = Utc::now();
        if let Some(response) = self.heartbeat_response(&value, now) {
            return Ok(vec![BitrueWsSessionEvent::HeartbeatResponse(response)]);
        }
        self.state.on_message(now);
        let message =
            parse_bitrue_public_stream_message(&self.exchange_id, self.symbol.clone(), &value)?;
        if matches!(message, BitruePublicStreamMessage::Pong) {
            self.state.on_pong(now);
        }
        let mut events = vec![BitrueWsSessionEvent::Public(message.clone())];
        if let BitruePublicStreamMessage::OrderBook(snapshot) = message {
            events.push(BitrueWsSessionEvent::Stream(
...
impl BitruePrivateWsSession {
    pub fn new(
        ...
        ensure_exchange_api_schema(subscription.schema_version)?;
        let subscribe_payload = bitrue_private_subscribe_payload(&subscription, market_type)?;
        let mut state = StreamRuntimeState::new(exchange_id.clone(), market_type);
        state.subscription_count = 1;
        Ok(Self {
            url: bitrue_private_listen_key_url(&url, listen_key, market_type)?,
            exchange_id,
            tenant_id: subscription.context.tenant_id.clone().ok_or_else(|| {
                ExchangeApiError::InvalidRequest {
                    message: "bitrue private ws session requires tenant_id".to_string(),
                }
            })?,
    ...
    pub fn handle_text_message(
        ...
        self.state.on_message(now);
        let message = parse_bitrue_private_stream_message(
            &self.exchange_id,
        ...
        if matches!(message, BitruePrivateStreamMessage::Pong) {
            self.state.on_pong(now);
        }
        let mut events = vec![BitrueWsSessionEvent::Private(message.clone())];
        if let BitruePrivateStreamMessage::Events(stream_events) = message {
            events.extend(stream_events.into_iter().map(BitrueWsSessionEvent::Stream));
        }
        Ok(events)
    }
}

fn bitrue_ws_symbol(symbol: &str, market_type: MarketType) -> ExchangeApiResult<String> {
    let symbol = normalize_bitrue_symbol(symbol, market_type)?;
...
fn parse_public_trade(
    ...
    symbol_hint: SymbolScope,
    value: &Value,
) -> ExchangeApiResult<BitruePublicTrade> {
    ...
    let canonical_symbol =
        symbol
            ...
            .ok_or_else(|| ExchangeApiError::InvalidRequest {
                message: "bitrue stream fill requires canonical_symbol".to_string(),
            })?;
...
fn normalize_order_event(value: &Value) -> Value {
    json!({
        ...
        "contractName": value_to_string(value.get("contractName")),
...
fn normalize_position_event(value: &Value) -> Value {
    ...
    if let Some(symbol) = value.get("s").or_else(|| value.get("symbol")).cloned() {
        object.entry("contractName".to_string()).or_insert(symbol);
    }
...
fn symbol_from_payload(
    ...
    let (base, quote) = split_bitrue_symbol(&exchange_symbol)
...
fn parse_ws_text(exchange_id: &ExchangeId, text: &str) -> ExchangeApiResult<Value> {
    ...
    if trimmed.is_empty() {
        return Err(parse_error(
            exchange_id.clone(),
            "empty bitrue websocket message",
            &Value::Null,
        ));
    }
    ...
    serde_json::from_str(trimmed).map_err(|error| {
        parse_error(
            exchange_id.clone(),
            &format!("invalid bitrue websocket JSON: {error}"),
            &json!({ "raw": trimmed }),
        )
    })
}
