...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("bitrue").expect("bitrue exchange")
}
...
pub(super) fn assert_signed_bitrue_request(request: &SeenRequest, method: &str, path: &str) {
    assert_eq!(request.method, method);
    assert_eq!(request.path, path);
    if path.starts_with("/fapi/") || path.starts_with("/user_stream/") {
        assert_eq!(
            request.headers.get("x-ch-apikey").map(String::as_str),
            Some("key")
        );
        assert!(request
            .headers
