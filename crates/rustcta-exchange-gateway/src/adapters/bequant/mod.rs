...
use rustcta_types::{AccountId, ExchangeId, MarketType, OrderType, TenantId};
...
pub use config::BequantGatewayConfig;
use transport::BequantRest;

#[derive(Clone)]
pub struct BequantGatewayAdapter {
    exchange_id: ExchangeId,
    config: BequantGatewayConfig,
    rest: BequantRest,
}

impl BequantGatewayAdapter {
    pub fn new(config: BequantGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("bequant").map_err(validation_error)?;
        let rest = BequantRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(BequantGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("bequant adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_spot(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "bequant.non_spot_market_type",
            });
        }
        Ok(())
    }
    ...
    async fn send_signed_post(
        ...
        let (api_key, api_secret) = self.private_credentials(operation)?;
        self.rest
            .send_signed_post(endpoint, params, form, api_key, api_secret)
            .await
    }
        ...
        let tenant_id =
            context
                ...
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "bequant private REST readback requires context.tenant_id".to_string(),
                })?;
        let account_id =
            context
                .account_id
                .clone()
                .ok_or_else(|| ExchangeApiError::InvalidRequest {
                    message: "bequant private REST readback requires context.account_id"
                        .to_string(),
                })?;
...
#[async_trait]
impl GatewayAdapter for BequantGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some("bequant spot REST gateway adapter".to_string()),
        }
    }
}

#[async_trait]
impl ExchangeClient for BequantGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = Some(
            streams::bequant_private_stream_capabilities(private_streams),
        );
        ...
        capabilities.capabilities_v2.private_streams = if private_streams {
            ...
            CapabilitySupport::rest_fallback(
                "bequant private websocket runtime is disabled by default; use REST reconciliation",
            )
        };
        capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
            ...
            private: if private_streams {
                ...
                CapabilitySupport::rest_fallback(
                    "bequant private order/fill/balance synchronization uses REST reconciliation",
                )
            },
        ...
        capabilities.capabilities_v2.batch_place_orders = BatchCapability {
            support: CapabilitySupport::unsupported(
                "bequant spot has no generic native batch create endpoint mapped to ExchangeClient",
            ),
        ...
        capabilities.capabilities_v2.batch_cancel_orders = BatchCapability {
            support: CapabilitySupport::unsupported(
                "bequant spot has cancel-all but no generic batch cancel-by-list endpoint mapped",
            ),
            mode: BatchExecutionMode::Unsupported,
            atomicity: BatchAtomicity::Unknown,
            max_items: None,
        ...
        capabilities.capabilities_v2.fills_history = HistoryCapability {
            support: if private_rest {
                ...
                CapabilitySupport::unsupported(
                    "bequant fills history requires private REST credentials",
                )
            },
            supports_since: true,
            supports_until: true,
        ...
        capabilities.capabilities_v2.order_history = HistoryCapability {
            support: if private_rest {
                ...
                CapabilitySupport::unsupported(
                    "bequant order history requires private REST credentials",
                )
            },
            supports_since: true,
            supports_until: true,
        ...
        capabilities.capabilities_v2.cancel_all_orders = if private_rest {
            ...
            CapabilitySupport::unsupported("bequant cancel-all requires private REST credentials")
        };
