...
use super::parser::{
    normalize_binanceus_symbol, normalize_depth, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl BinanceUsGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        let requested = request
            .symbols
            .iter()
            .map(|symbol| normalize_binanceus_symbol(&symbol.exchange_symbol.symbol))
...
fn retain_requested_symbols(response: &mut Value, requested: &[String]) {
    ...
    symbols.retain(|symbol| {
        symbol
            ...
            .and_then(|symbol| normalize_binanceus_symbol(symbol).ok())
