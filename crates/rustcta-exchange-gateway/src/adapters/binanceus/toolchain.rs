...
const BINANCEUS_PROFILE_UNMAPPED: &str =
    "Binance.US profile does not expose a verified equivalent for this Binance.com adapter operation";
...
pub(super) fn apply_toolchain_capabilities(
    ...
    capabilities.capabilities_v2.private_rest = if private_rest_enabled {
        ...
        CapabilitySupport::unsupported(
            "Binance.US private REST requires enabled API key and secret",
        )
    };
    capabilities.capabilities_v2.public_streams = CapabilitySupport::unsupported(
        "Binance.US public WS runtime is not wired in this adapter yet",
    );
    capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
        "Binance.US private user data stream is not wired; use REST reconciliation through account, open orders, query order, and myTrades",
    );
    capabilities.capabilities_v2.batch_place_orders =
        BatchCapability::unsupported("Binance.US batch place order is not implemented");
    capabilities.capabilities_v2.batch_cancel_orders =
        BatchCapability::unsupported("Binance.US batch cancel order is not implemented");
    capabilities.capabilities_v2.cancel_all_orders = if private_rest_enabled {
        CapabilitySupport::native()
    } else {
        CapabilitySupport::unsupported("Binance.US cancel-all requires private REST credentials")
    };
    capabilities.capabilities_v2.order_history = HistoryCapability {
        support: if private_rest_enabled {
            ...
            CapabilitySupport::unsupported(
                "Binance.US order history requires private REST credentials",
            )
        },
    ...
    capabilities.capabilities_v2.fills_history = HistoryCapability {
        support: if private_rest_enabled {
            ...
            CapabilitySupport::unsupported("Binance.US myTrades requires private REST credentials")
        },
        supports_since: true,
        supports_until: true,
        supports_limit: true,
...
fn endpoint_capabilities(private_rest_enabled: bool) -> Vec<EndpointCapability> {
    ...
    for (operation, path, credential_scope) in [
        (
            "amend_order",
            "/unsupported/binanceus/amend_order",
            CredentialScope::Trade,
        ),
        (
            "place_order_list",
        ...
        endpoints.push(rest_endpoint(
            operation,
            CapabilitySupport::unsupported(BINANCEUS_PROFILE_UNMAPPED),
            "UNSUPPORTED",
            path,
            EndpointAuth::None,
            vec![credential_scope],
