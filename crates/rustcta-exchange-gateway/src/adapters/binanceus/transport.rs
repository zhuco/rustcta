...
#[derive(Clone)]
pub struct BinanceUsPublicRest {
    exchange_id: ExchangeId,
    rest_base_url: String,
    http: reqwest::Client,
}

impl BinanceUsPublicRest {
    pub fn new(
        exchange_id: ExchangeId,
        rest_base_url: String,
...
async fn parse_response(
    ...
    if !status.is_success() {
        ...
        let message = value
            ...
            .unwrap_or("Binance.US request failed");
        let mut error = ExchangeError::new(
            exchange_id,
            classify_binanceus_error(code.as_deref(), message),
            message,
            Utc::now(),
        );
        error.code = code;
...
fn classify_binanceus_error(code: Option<&str>, message: &str) -> ExchangeErrorClass {
