...
use super::{BinanceUsGatewayAdapter, BinanceUsGatewayConfig};

fn fixture(path: &str) -> serde_json::Value {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/binanceus/{path}",
...
#[tokio::test]
async fn binanceus_adapter_should_declare_task5_profile_capabilities() {
    let adapter = BinanceUsGatewayAdapter::new(BinanceUsGatewayConfig {
        api_key: Some("key".to_string()),
        api_secret: Some("secret".to_string()),
        enabled_private_rest: true,
        ..BinanceUsGatewayConfig::default()
    })
    .expect("adapter");
...
#[test]
fn binanceus_parser_fixtures_should_cover_success_empty_and_error_shapes() {
...
#[tokio::test]
async fn binanceus_adapter_should_load_symbol_rules_from_public_rest() {
...
#[tokio::test]
async fn binanceus_adapter_should_ignore_unrequested_non_ascii_symbol_rules() {
    let (base_url, seen) = spawn_rest_server(vec![json!({
        "symbols": [
            {
                "symbol": "BTCUSDT",
...
#[tokio::test]
async fn binanceus_adapter_should_load_depth_limited_order_book_from_public_rest() {
    let (base_url, seen) = spawn_rest_server(vec![json!({
