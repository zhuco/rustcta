...
pub(super) fn apply_toolchain_capabilities(capabilities: &mut ExchangeClientCapabilities) {
    ...
    capabilities.capabilities_v2.private_rest = CapabilitySupport::unsupported(
        "WavesExchange private reads and writes require Waves public-key/order signing semantics",
    );
    capabilities.capabilities_v2.public_streams = CapabilitySupport::unsupported(
        "WavesExchange public WebSocket payloads are spec-only until runtime resync is implemented",
    );
    capabilities.capabilities_v2.private_streams = CapabilitySupport::unsupported(
        "WavesExchange address streams require wallet-scoped subscription and are not mapped",
    );
    capabilities.capabilities_v2.stream_runtime = StreamRuntimeCapability {
        public: CapabilitySupport::unsupported(
            "WavesExchange public WS is payload/parser-only in this adapter",
        ),
        private: CapabilitySupport::unsupported("WavesExchange private WS auth is not mapped"),
...
fn endpoint_capabilities() -> Vec<EndpointCapability> {
    let mut endpoints = vec![
        EndpointCapability {
            ...
            path: Some("/matcher/orderbook/{amountAsset}/{priceAsset}/info".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("wavesexchange_public".to_string()),
            weight: Some(1),
        ...
        EndpointCapability {
            ...
            transport: EndpointTransport::Rest,
            method: Some("GET".to_string()),
            path: Some("/matcher/orderbook/{amountAsset}/{priceAsset}".to_string()),
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("wavesexchange_public".to_string()),
            weight: Some(1),
    ...
    endpoints.extend(
        ...
        .map(|(operation, reason)| EndpointCapability {
            ...
            auth: EndpointAuth::None,
            credential_scopes: Vec::new(),
            rate_limit_bucket: Some("wavesexchange_unsupported".to_string()),
            weight: Some(0),
