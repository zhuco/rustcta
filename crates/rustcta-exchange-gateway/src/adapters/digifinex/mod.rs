...
pub use config::DigiFinexGatewayConfig;
use transport::DigiFinexRest;

#[derive(Clone)]
pub struct DigiFinexGatewayAdapter {
    exchange_id: ExchangeId,
    config: DigiFinexGatewayConfig,
    rest: DigiFinexRest,
}

impl DigiFinexGatewayAdapter {
    pub fn new(config: DigiFinexGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id = ExchangeId::new("digifinex").map_err(validation_error)?;
        let rest = DigiFinexRest::new(
            exchange_id.clone(),
    ...
    pub fn default_public() -> ExchangeApiResult<Self> {
        Self::new(DigiFinexGatewayConfig::default())
    }

    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!("digifinex adapter cannot serve request for exchange {exchange}"),
            });
        }
        Ok(())
    }
    ...
    fn ensure_supported_market(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if !matches!(market_type, MarketType::Spot | MarketType::Perpetual) {
            return Err(ExchangeApiError::Unsupported {
                operation: "digifinex.unsupported_market_type",
            });
        }
        Ok(())
    }
...
#[async_trait]
impl GatewayAdapter for DigiFinexGatewayAdapter {
        ...
        GatewayExchangeStatus {
            ...
            message: Some(
                if self.config.private_rest_enabled() {
                    "digifinex spot + perpetual swap public/private REST + WS request-spec gateway adapter"
                } else {
                    "digifinex spot + perpetual swap public REST + WS request-spec gateway adapter"
                }
                .to_string(),
            ),
        }
...
#[async_trait]
impl ExchangeClient for DigiFinexGatewayAdapter {
    ...
    async fn amend_order(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "digifinex.amend_order",
        })
    }
    ...
    async fn place_order_list(
        ...
        Err(ExchangeApiError::Unsupported {
            operation: "digifinex.place_order_list",
        })
    }
