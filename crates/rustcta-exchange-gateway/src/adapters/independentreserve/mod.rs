...
pub use config::IndependentReserveGatewayConfig;
use signing::IndependentReservePrivateCredentials;
use transport::IndependentReserveRest;

#[derive(Clone)]
pub struct IndependentReserveGatewayAdapter {
    exchange_id: ExchangeId,
    config: IndependentReserveGatewayConfig,
    rest: IndependentReserveRest,
}

impl IndependentReserveGatewayAdapter {
    pub fn new(config: IndependentReserveGatewayConfig) -> ExchangeApiResult<Self> {
        let exchange_id =
            ExchangeId::new("independentreserve").map_err(parser::validation_error)?;
        let rest = IndependentReserveRest::new(
            exchange_id.clone(),
            config.rest_base_url.clone(),
            config.request_timeout_ms,
            IndependentReservePrivateCredentials::from_config(&config),
        )?;
        Ok(Self {
            exchange_id,
            config,
    ...
    fn ensure_exchange(&self, exchange: &ExchangeId) -> ExchangeApiResult<()> {
        if exchange != &self.exchange_id {
            return Err(ExchangeApiError::InvalidRequest {
                message: format!(
                    "independentreserve adapter cannot serve request for exchange {exchange}"
                ),
            });
        }
        Ok(())
    ...
    fn ensure_supported_market_type(&self, market_type: MarketType) -> ExchangeApiResult<()> {
        if market_type != MarketType::Spot {
            return Err(ExchangeApiError::Unsupported {
                operation: "independentreserve.unsupported_market_type",
            });
        }
        Ok(())
    }

    fn ensure_supported_quote(&self, symbol: &str) -> ExchangeApiResult<()> {
        let (_, quote) = parser::split_symbol_assets(symbol);
        if matches!(quote.as_str(), "AUD" | "SGD" | "USD") {
            return Ok(());
        }
        Err(ExchangeApiError::Unsupported {
            operation: "independentreserve.unsupported_fiat_quote",
        })
    }
...
#[async_trait]
impl GatewayAdapter for IndependentReserveGatewayAdapter {
    fn gateway_exchange_status(&self) -> GatewayExchangeStatus {
        GatewayExchangeStatus {
            ...
            message: Some(
                "independentreserve AUD/SGD/USD spot REST/WS gateway adapter".to_string(),
            ),
        }
    }
}
...
#[async_trait]
impl ExchangeClient for IndependentReserveGatewayAdapter {
    ...
    fn capabilities(&self) -> ExchangeClientCapabilities {
        ...
        capabilities.private_stream_capabilities = Some(if private {
            streams::independentreserve_private_stream_capabilities()
        } else {
