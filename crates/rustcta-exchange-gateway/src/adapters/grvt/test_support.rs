...
pub fn grvt_exchange_id() -> ExchangeId {
    ExchangeId::new("grvt").expect("exchange")
}

pub fn grvt_context(request_id: &str) -> RequestContext {
    RequestContext {
...
pub fn grvt_symbol(market_type: MarketType) -> SymbolScope {
    SymbolScope {
        exchange: grvt_exchange_id(),
        market_type,
        canonical_symbol: Some(CanonicalSymbol::new("BTC", "USDT").expect("canonical")),
        exchange_symbol: ExchangeSymbol::new(grvt_exchange_id(), market_type, "BTC_USDT_Perp")
            .expect("symbol"),
    }
}
