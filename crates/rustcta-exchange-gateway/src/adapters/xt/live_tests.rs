...
#[tokio::test]
#[ignore]
async fn xt_live_readonly_should_cover_public_and_authenticated_baseline() {
    let Some(api_key) = non_empty_env("XT_API_KEY") else {
        eprintln!("XT_API_KEY missing; skipping XT authenticated live readonly test");
        return;
    };
    let Some(api_secret) = non_empty_env("XT_API_SECRET") else {
        eprintln!("XT_API_SECRET missing; skipping XT authenticated live readonly test");
        return;
    };
    ...
    let spot_book = adapter
        .get_order_book(OrderBookRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("xt-live-spot-book"),
    ...
    let perp_book = adapter
        .get_order_book(OrderBookRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("xt-live-perp-book"),
    ...
    adapter
        .get_balances(BalancesRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("xt-live-balances"),
    ...
    adapter
        .get_open_orders(OpenOrdersRequest {
            schema_version: EXCHANGE_API_SCHEMA_VERSION,
            context: context("xt-live-open-orders"),
            exchange: exchange_id(),
            market_type: Some(MarketType::Spot),
