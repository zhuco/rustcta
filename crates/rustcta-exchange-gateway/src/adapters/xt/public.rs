...
use super::parser::{
    normalize_depth, normalize_xt_symbol, parse_orderbook_snapshot, parse_symbol_rules,
};
...
impl XtGatewayAdapter {
    pub(super) async fn get_symbol_rules_impl(
        ...
        for market_type in markets {
            ...
            let response = self
                .rest
                .send_public_request(market_type, endpoint, &HashMap::new())
                .await?;
    ...
    pub(super) async fn get_order_book_impl(
        ...
        let endpoint = match request.symbol.market_type {
            MarketType::Spot => "/v4/public/depth",
            MarketType::Perpetual => "/future/market/v1/public/q/depth",
            _ => unreachable!("checked by ensure_supported_market"),
        };
        if request.symbol.market_type == MarketType::Perpetual {
            let limit = params.remove("limit").unwrap_or_else(|| "20".to_string());
            params.insert("level".to_string(), limit);
        }
        let value = self
            .rest
