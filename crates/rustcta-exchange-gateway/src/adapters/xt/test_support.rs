...
pub(super) fn exchange_id() -> ExchangeId {
    ExchangeId::new("xt").expect("xt exchange")
}
...
pub(super) fn assert_signed_xt_request(request: &SeenRequest, method: &str, path: &str) {
    assert_eq!(request.method, method);
    assert_eq!(request.path, path);
    assert_eq!(
        request.headers.get("validate-appkey").map(String::as_str),
        Some("key")
    );
    assert!(request
        .headers
...
pub(super) fn request_spec(name: &str) -> crate::request_spec::RequestSpec {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/xt/request_specs/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("XT request spec fixture"))
        .expect("XT request spec json")
}

pub(super) fn signing_vector(name: &str) -> crate::signing_spec::SigningVector {
    let path = format!(
        "{}/../../tests/fixtures/exchanges/xt/signing_vectors/{name}.json",
        env!("CARGO_MANIFEST_DIR")
    );
    serde_json::from_str(&std::fs::read_to_string(path).expect("XT signing vector fixture"))
        .expect("XT signing vector json")
}
