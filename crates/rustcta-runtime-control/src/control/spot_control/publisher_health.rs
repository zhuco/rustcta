...
use rustcta_types::{
    ExchangeId, RouteHealth, RouteHealthStatus, RuntimePublisherSnapshot, RuntimePublisherStatus,
};
...
impl RuntimePublisherHealth {
    pub fn to_runtime_publisher_snapshot(
        &self,
        publisher_id: impl Into<String>,
        observed_at: DateTime<Utc>,
    ) -> Result<RuntimePublisherSnapshot, rustcta_types::ValidationError> {
        let mut snapshot =
            RuntimePublisherSnapshot::new(publisher_id, "typed_runtime_snapshot", observed_at)?;
        snapshot.running = self.running;
        snapshot.last_snapshot_at = self.last_snapshot_at;
        snapshot.snapshots_generated = self.snapshots_generated;
        snapshot.snapshots_persisted = self.snapshots_persisted;
        snapshot.status = self.runtime_status(observed_at);
        snapshot.errors = self
            .last_critical_error
            .iter()
            .cloned()
            .chain(
                self.per_exchange_health
                    .values()
                    .filter_map(|health| health.last_error.clone()),
            )
            .collect();
        snapshot.route_health = self
            .per_exchange_health
            .values()
            .filter_map(|health| health.to_route_health(observed_at).ok())
            .collect();
        Ok(snapshot)
    }

    fn runtime_status(&self, observed_at: DateTime<Utc>) -> RuntimePublisherStatus {
        if !self.running {
            return RuntimePublisherStatus::Stopped;
        }
        if self.last_critical_error.is_some() || self.snapshot_persist_failures > 0 {
            return RuntimePublisherStatus::Error;
        }
        let stale = self.last_snapshot_at.is_some_and(|last_snapshot_at| {
            observed_at
                .signed_duration_since(last_snapshot_at)
                .num_seconds()
                > 60
        });
        if stale {
            RuntimePublisherStatus::Stale
        } else if self
            .per_exchange_health
            .values()
            .any(|health| health.last_error.is_some() || health.backoff_until.is_some())
        {
            RuntimePublisherStatus::Degraded
        } else {
            RuntimePublisherStatus::Ok
        }
    }
}
...
impl RuntimePublisherExchangeHealth {
    pub fn to_route_health(
        &self,
        observed_at: DateTime<Utc>,
    ) -> Result<RouteHealth, rustcta_types::ValidationError> {
        let route_id = format!("runtime-publisher:{}", self.exchange);
        let status = if self.last_error.is_some() {
            RouteHealthStatus::Degraded
        } else if self.last_successful_request.is_some() {
            RouteHealthStatus::Healthy
        } else {
            RouteHealthStatus::Unknown
        };
        let mut route = RouteHealth::new(route_id, "runtime_publisher", status, observed_at)?;
        route.exchange_id = ExchangeId::new(&self.exchange).ok();
        route.last_success_at = self.last_successful_request;
        route.last_error_at = self.last_error.as_ref().map(|_| observed_at);
        route.last_error = self.last_error.clone();
        route.latency_ms = self
            .request_latency_ms
...
mod tests {
    ...
    #[test]
    fn publisher_health_should_export_root_free_publisher_snapshot() {
        let now = Utc::now();
        let health = RuntimePublisherHealth {
            running: true,
            last_snapshot_at: Some(now),
            snapshots_generated: 2,
            snapshots_persisted: 1,
            per_exchange_health: BTreeMap::from([(
                "gateio".to_string(),
                RuntimePublisherExchangeHealth {
            ...
            ..RuntimePublisherHealth::default()
        };

        let snapshot = health
            .to_runtime_publisher_snapshot("spot-control", now)
            .expect("publisher snapshot");

        assert_eq!(snapshot.status, RuntimePublisherStatus::Ok);
        assert_eq!(snapshot.route_health[0].status, RouteHealthStatus::Healthy);
        assert_eq!(snapshot.route_health[0].latency_ms, Some(42));
    }
}
