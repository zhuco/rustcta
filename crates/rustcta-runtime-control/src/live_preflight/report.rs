...
mod tests {
    ...
    use super::*;
    use crate::live_preflight::{LivePreflightGate as Gate, LivePreflightSeverity as Severity};

    #[test]
    fn report_should_aggregate_scoped_readiness_without_secrets() {
        let report = build_report(
            ...
            vec![
                LivePreflightCheck::new(
                    ...
                    Severity::Info,
                    "read permission present",
                )
                .scoped(Some("MEXC"), Some(MarketType::Spot), Some("btc/usdt"))
                .detail("permission", "read"),
                LivePreflightCheck::new(
                    Gate::WebsocketBooks,
                    "fresh_book",
                    LivePreflightCheckStatus::Warn,
                    Severity::Warning,
                    "book is close to stale threshold",
                )
                .scoped(Some("MEXC"), Some(MarketType::Spot), Some("btc/usdt")),
            ],
        );

        assert_eq!(report.decision, LiveReadinessDecision::ReadyForSmallLive);
        assert_eq!(
            report.per_exchange_readiness.get("mexc"),
            Some(&LivePreflightCheckStatus::Warn)
        );
        assert_eq!(
            report.per_symbol_readiness.get("mexc:BTC/USDT"),
            Some(&LivePreflightCheckStatus::Warn)
        );
